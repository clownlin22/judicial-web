...
@Controller
@RequestMapping("bacera/ctDna")
public class RdsBaceraCTDnaController extends RdsBaceraAbstractController {

	@Setter
	@Autowired
	private RdsBaceraCTDnaService rdsCTDnaService;
	...
	public Object queryAllPage(@RequestBody Map<String, Object> params)
			throws Exception {
		return rdsCTDnaService.queryAllPage(params);
	}
	...
	private Object insert(Map<String, Object> params,HttpServletRequest request) {
		try {
			int numResult = rdsCTDnaService.queryNumExit(params);
			if (numResult > 0) {
			...
			//更新基本信息
			int result = rdsCTDnaService.insert(params);
			if (result > 0) {
	...
	private Object update(Map<String, Object> params,HttpServletRequest request) {
		try {
			...
			if (numResult <= 0) {
				...
				if (rdsCTDnaService.queryNumExit(map1) > 0)
			...
			//更新基本信息
			int result = rdsCTDnaService.update(params);
			if (result > 0) {
	...
	public void exportChildPCR(HttpServletResponse response, HttpServletRequest request, HttpSession session) {
		try {
			...
			params.put("ctDNA_starttime", request.getParameter("ctDNA_starttime"));
			params.put("ctDNA_endtime", request.getParameter("ctDNA_endtime"));
			...
			rdsCTDnaService.exportCTDna(params, response);;
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
