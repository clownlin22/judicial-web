var exportKit='';
Ext.define("Rds.statistic.panel.FinanceKitSetGridPanel",{
...
//							mode: 'SINGLE'
				       ...
				       var pagingToolbar=Ext.getCmp('pagingbarKitSet');
						...
						me.dockedItems = [
								{
									...
									style : {
							 		...
							 		items:[{
									 			...
									 			text:'案例确认',
									 			iconCls:'Pageedit',
									 			handler:me.confirmList
									 		},{
												text : '导出',
												iconCls : 'Pageexcel',
												handler : me.onKitExport
											}]
								}];
						me.callParent(arguments);
					},
					onKitExport:function(){
						window.location.href = "statistics/kitSet/exportKit.do"+exportKit;
					},
					confirmList:function(){
						...
						{
							ids += selections[i].get("kit_id")+",";
							if(selections[i].get("confirm_state")=='2' || selections[i].get("is_delete")=='2'){
								Ext.Msg.alert("提示", "该案例状态不允许确认!");
								return;
							}
						}
						...
						Ext.MessageBox.confirm("提示", "确认选中记录？", function (btn) {
					        if("yes"==btn)
					        {
					    		...
					    		Ext.Ajax.request({
					    			url : "statistics/kitSet/comfirmKitSet.do",
					    			method : "POST",
					    			headers : {
					...
					onDelete:function(){
						...
						for(var i = 0 ; i < selections.length ; i ++)
						{
							ids += selections[i].get("kit_id")+",";
							if(selections[i].get("confirm_state")=='2' || selections[i].get("is_delete")=='2'){
								Ext.Msg.alert("提示", "该案例状态不允许删除!");
								return;
							}
						}
						ids = ids.substring(0,ids.length-1);
						Ext.MessageBox.confirm("提示", "确认删除记录？", function (btn) {
					        if("yes"==btn)
					        {
					        	Ext.MessageBox.wait('正在操作','请稍后...'); 	
					    		Ext.Ajax.request({
					    			url : "statistics/kitSet/deleteKitSet.do",
					    			method : "POST",
					    			headers : {
					    				'Content-Type' : 'application/json'
					    			},
					...
					onInsert:function(){
						var me = this.up("gridpanel");
						var form = Ext.create("Rds.statistic.form.FinanceKitSetForm",{
							region:"center",
							autoScroll : true,
							grid:me
						});
					...
					onUpdate:function(){
						...
						if(selections[0].get("confirm_state") == '2' || selections[0].get("is_delete") == '2')
						{
							Ext.Msg.alert("提示", "该记录不能修改!");
							return;
						}
						var form = Ext.create("Rds.statistic.form.FinanceKitSetForm",{
							region:"center",
