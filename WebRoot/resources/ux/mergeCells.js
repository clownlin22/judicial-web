...
function mergeCells(grid, cols) {
	...
	// var arrayTr =
	// document.getElementById(grid.getId()+"-body").firstChild.nextSibling;
	// var arrayTr =
	// Ext.get(grid.getId()+"-body").first().first().first().select("tr");
	...
	Ext.each(cols, function(colIndex) { // 逐列去操作tr
		...
		for ( var i = 1; i < trCount; i++) { // i=0表示表头等没用的行
			...
			var cold = 0;
			Ext.each(arrayTd,
					function(Td) { // 获取RowNumber列和check列
						if (Td.getAttribute("class").indexOf(
								"x-grid-cell-special") != -1)
							cold++;
					});
