var hpv="";
Ext.define('Rds.bacera.panel.BaceraHpvListGrid', {
	...
	initComponent : function() {
		...
		var hpv_starttime = new Ext.form.DateField({
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('hpv_starttime').getValue();
		            var endDate = Ext.getCmp('hpv_endtime').getValue();
		            if (start > endDate) {
		                Ext.getCmp('hpv_starttime').setValue(endDate);
		            }
				}
			}
		});
		...
		me.store = Ext.create('Ext.data.Store',{
			fields:['id','num','name','age','gender','date','ownpersonname','program',
            ...
            listeners:{
            	'beforeload':function(ds, operation, opt){
            		...
            		hpv = "num=" + search.getValue().trim() + "&name="
							+ name.getValue().trim() + "&ownperson="
							+ ownperson.getValue().trim() + "&hpv_starttime="
							+ dateformat(hpv_starttime.getValue())
							+ "&hpv_endtime="
							+ dateformat(hpv_endtime.getValue())
							+ "&reportif=" + reportif.getValue()
							+ "&cancelif=" + cancelif.getValue() + "&agent="
							+ agent.getValue().trim() + "&areacode="
							+ areacode.getValue() +"&gender="
							+ gender.getValue();
       ...
       var pagingToolbar=Ext.getCmp('pagingbarHPV');
...
//		me.bbar = {xtype: 'label',id:'totalBbarHpv', text: '',
//				style:'height:25px;line-height:25px;text-align:right;margin-right:10px;'};
		me.columns = [
	              { text: '案例编号',	dataIndex: 'num', menuDisabled:true, width : 100,
	                  ...
	                  { text: '项目',	dataIndex: 'program', menuDisabled:true, width : 80},
		...
		me.dockedItems = [{
	        ...
	        borderBottomWidth : '0px !important'
	 		...
	 		name:'search',
	 		dock:'top',
	 		items:[reportif,hpv_starttime,hpv_endtime,cancelif,program]
	 	},{
	 		style : {
	        borderTopWidth : '0px !important',
	        borderBottomWidth : '0px !important'
...
//			Ext.getCmp('totalBbarHpv').setText("共 "+me.store.getCount()+" 条");
//		});
	},
	hpvExport:function(){
		window.location.href = "bacera/hpv/exportHpv.do?"+hpv ;
	},
	//新增
	onInsert:function(){
		...
		ownpersonTemp = "";
		confirm_flagTemp="";
		var form = Ext.create("Rds.bacera.form.BaceraHpvListForm",{
			region:"center",
			grid:me
		});
