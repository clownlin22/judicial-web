...
Ext.define('Rds.bacera.panel.BaceraTumorPreFinanceGrid', {
	...
	initComponent : function() {
		...
		var tumorPer_starttime = new Ext.form.DateField({
			id:'tumorPer_starttime_finance',
			name : 'tumorPer_starttime_finance',
        	width:'20%',
			fieldLabel : '日 期  从',
			labelWidth : 50,
			labelAlign : 'right',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('tumorPer_starttime_finance').getValue();
	                var endDate = Ext.getCmp('tumorPer_endtime_finance').getValue();
	                if (start > endDate) {
	                    Ext.getCmp('tumorPer_starttime_finance').setValue(endDate);
	                }
				}
			}
		});
		var tumorPer_endtime = new Ext.form.DateField({
			id:'tumorPer_endtime_finance',
			name : 'tumorPer_endtime_finance',
        	width:'20%',
			labelWidth : 20,
			fieldLabel : ' 到 ',
			labelAlign : 'right',
...
//			maxValue : new Date(),
		...
		var paragraphtime_starttime = new Ext.form.DateField({
			id:'tumorPer_starttime_finance_paragraphtime',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('tumorPer_starttime_finance_paragraphtime').getValue();
					var endDate = Ext.getCmp('tumorPer_endtime_finance_paragraphtime').getValue();
					if (start > endDate) {
						Ext.getCmp('tumorPer_starttime_finance_paragraphtime').setValue(endDate);
					}
				}
			}
		});
		var paragraphtime_endtime = new Ext.form.DateField({
			id:'tumorPer_endtime_finance_paragraphtime',
		...
		me.store = Ext.create('Ext.data.Store',{
			fields:['id','num','name','age','testitems','diagnosis','checkper','agentname','ownpersonname',
			        'ownperson','reportif','receivables','payment','paid','paragraphtime','discountPrice',
			        'account_type','account_type','reportif','hospital','expresstype','expressnum','confirm_flag',
			        'recive','phonenum','date','remark','cancelif','remarks','remittanceName','remittanceDate'],
	        start:0,
			limit:15,
			pageSize:15,
			proxy: {
...
//			mode: 'SINGLE'
		...
		//分页的combobox下拉选择显示条数
	     combo = Ext.create('Ext.form.ComboBox',{
	        ...
	        var pagingToolbar=Ext.getCmp('pagingbarTumorPerFinance');
...
//		me.bbar = {xtype: 'label',id:'totalBbarTumorPer_finance', text: '',style:'height:25px;line-height:25px;text-align:right;margin-right:10px;'};
		me.columns = [
		...
		me.dockedItems = [{
	        ...
	        borderBottomWidth : '0px !important'
	 		...
	 		name:'search',
	 		dock:'top',
	 		items:[reportif,tumorPer_starttime,tumorPer_endtime,checkper,hospital]
	 	},{
	 		style : {
		        borderTopWidth : '0px !important',
...
//			Ext.getCmp('totalBbarTumorPer_finance').setText("共 "+me.store.getCount()+" 条");
//		});
	},
