...
Ext.define('Rds.bacera.panel.BaceraTumorPreExpressGrid', {
	...
	initComponent : function() {
		...
		var tumorPer_starttime = new Ext.form.DateField({
			id:'tumorPer_starttime_express',
			name : 'tumorPer_starttime_express',
        	width:'20%',
			fieldLabel : '日 期  从',
			labelWidth : 50,
			emptyText : '请选择日期',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('tumorPer_starttime_express').getValue();
	                var endDate = Ext.getCmp('tumorPer_endtime_express').getValue();
	                if (start > endDate) {
	                    Ext.getCmp('tumorPer_starttime_express').setValue(endDate);
	                }
				}
			}
		});
		...
		var tumorPer_endtime = new Ext.form.DateField({
			id:'tumorPer_endtime_express',
			name : 'tumorPer_endtime_express',
        	width:'20%',
			labelWidth : 20,
			fieldLabel : ' 到 ',
			labelAlign : 'right',
		...
		me.store = Ext.create('Ext.data.Store',{
			fields:['id','num','name','age','testitems','diagnosis','checkper','agentname','ownpersonname',
			        'ownperson','paragraphtime','account_type','reportif','hospital','remarks',
		...
		me.bbar = {xtype: 'label',id:'totalBbarTumorPer_express', text: '',
		...
		me.dockedItems = [{
	        ...
	        borderBottomWidth : '0px !important'
	 		...
	 		name:'search',
	 		dock:'top',
	 		items:[reportif,expressnum,tumorPer_starttime,tumorPer_endtime,checkper]
	 	},{
	 		style : {
		        borderTopWidth : '0px !important',
		        borderBottomWidth : '1px !important'
		 		...
		 		name:'search',
		 		dock:'top',
		 		items:[express_starttime,express_endtime,hospital,{
	            	text:'查询',
	            	iconCls:'Find',
	            	handler:me.onSearch
	            }]
		...
		me.store.on("load",function(){
			Ext.getCmp('totalBbarTumorPer_express').setText("共 "+me.store.getCount()+" 条");
		});
	},
	onSearch:function(){
