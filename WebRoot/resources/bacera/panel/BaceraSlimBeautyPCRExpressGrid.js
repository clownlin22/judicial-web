...
Ext.define('Rds.bacera.panel.BaceraSlimBeautyPCRExpressGrid', {
	...
	initComponent : function() {
		...
		var slimBeautyPCR_starttime = new Ext.form.DateField({
			id:'slimBeautyPCR_starttime_express',
			name : 'slimBeautyPCR_starttime_express',
        	width:'20%',
			fieldLabel : '日 期  从',
			labelWidth : 70,
			emptyText : '请选择日期',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('slimBeautyPCR_starttime_express').getValue();
	                var endDate = Ext.getCmp('slimBeautyPCR_endtime_express').getValue();
	                if (start > endDate) {
	                    Ext.getCmp('slimBeautyPCR_starttime_express').setValue(endDate);
	                }
				}
			}
		});
		var slimBeautyPCR_endtime = new Ext.form.DateField({
			id:'slimBeautyPCR_endtime_express',
			name : 'slimBeautyPCR_endtime_express',
        	width:'20%',
			labelWidth : 20,
			fieldLabel : ' 到 ',
			labelAlign : 'right',
...
//			maxValue : new Date(),
		...
		me.store = Ext.create('Ext.data.Store',{
				fields:['id','num','name','age','testitems','gender','checkper','paragraphtime','account_type',
				        ...
				        'remarks','agentname','reportif','remark','cancelif','expresstime','reportType',
				        'orderPlatform','inspectionUnits','case_type'],
			    proxy: {
                type: 'jsonajax',
		...
		me.bbar = {xtype: 'label',id:'totalBbarSlimBeautyPCR_express', text: '',
		...
		me.store.on("load",function(){
			Ext.getCmp('totalBbarSlimBeautyPCR_express').setText("共 "+me.store.getCount()+" 条");
		});
	},
	onSearch:function(){
