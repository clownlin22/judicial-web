...
Ext.define('Rds.bacera.panel.BaceraWuHanMedExamineFinanceGrid', {
	...
	initComponent : function() {
		...
		var medExamine_starttime = new Ext.form.DateField({
			id:'wuHanMedExamine_starttime_finance',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('wuHanMedExamine_starttime_finance').getValue();
	                var endDate = Ext.getCmp('wuHanMedExamine_endtime_finance').getValue();
	                if (start > endDate) {
	                    Ext.getCmp('wuHanMedExamine_starttime_finance').setValue(endDate);
	                }
				}
			}
		});
		var medExamine_endtime = new Ext.form.DateField({
			id:'wuHanMedExamine_endtime_finance',
		...
		var paragraphtime_starttime = new Ext.form.DateField({
			id:'wuHanMedExamine_starttime_finance_paragraphtime',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('wuHanMedExamine_starttime_finance_paragraphtime').getValue();
					var endDate = Ext.getCmp('wuHanMedExamine_endtime_finance_paragraphtime').getValue();
					if (start > endDate) {
						Ext.getCmp('wuHanMedExamine_starttime_finance_paragraphtime').setValue(endDate);
					}
				}
			}
		});
		var paragraphtime_endtime = new Ext.form.DateField({
			id:'wuHanMedExamine_endtime_finance_paragraphtime',
...
//			forceSelection: true, 
		...
		me.store = Ext.create('Ext.data.Store',{
			fields:['id','num','name','program','date','ownpersonname',
			        'ownperson','agentname','reportif','remark','cancelif', 'receivables','payment',
			        'paid','discountPrice','paragraphtime','account_type','confirm_flag',
			        'areaname','remarks','remittanceName','remittanceDate'],
	        start:0,
			limit:15,
			pageSize:15,
			proxy: {
...
//			mode: 'SINGLE'
		...
		//分页的combobox下拉选择显示条数
	     combo = Ext.create('Ext.form.ComboBox',{
	        ...
	        var pagingToolbar=Ext.getCmp('pagingbarWuHanMedFinance');
...
//		me.bbar = {xtype: 'label',id:'totalBbarMedExamineWuHan_finance', text: '',style:'height:25px;line-height:25px;text-align:right;margin-right:10px;'};
		me.columns = [
		...
		me.dockedItems = [{
	 		...
	 		name:'search',
	 		dock:'top',
	 		items:[search,name,ownperson,areacode,program]
	 	},{
	 		style : {
	        ...
	        borderBottomWidth : '0px !important'
	 		...
	 		name:'search',
	 		dock:'top',
	 		items:[medExamine_starttime,medExamine_endtime,paragraphtime_starttime,paragraphtime_endtime]
	 	},{
	 		style : {
		        borderTopWidth : '0px !important',
		        borderBottomWidth : '0px !important'
...
//			Ext.getCmp('totalBbarMedExamineWuHan_finance').setText("共 "+me.store.getCount()+" 条");
//		});
	},	
	caseFeeAdd:function(){
