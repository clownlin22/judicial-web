...
Ext.define('Rds.bacera.panel.BaceraForensicFinancetGrid', {
	...
	initComponent : function() {
		...
		var forensic_starttime = new Ext.form.DateField({
			id:'forensic_starttime_finance',
			name : 'forensic_starttime_finance',
        	width:'20%',
			fieldLabel : '收案时间从',
			labelWidth : 70,
			labelAlign : 'right',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('forensic_starttime_finance').getValue();
	                var endDate = Ext.getCmp('forensic_endtime_finance').getValue();
	                if (start > endDate) {
	                    Ext.getCmp('forensic_starttime_finance').setValue(endDate);
	                }
				}
			}
		});
		var forensic_endtime = new Ext.form.DateField({
			id:'forensic_endtime_finance',
			name : 'forensic_endtime_finance',
        	width:'20%',
			labelWidth : 20,
			fieldLabel : ' 到 ',
			labelAlign : 'right',
...
//			maxValue : new Date(),
		...
		var paragraphtime_starttime = new Ext.form.DateField({
			id:'forensic_starttime_finance_paragraphtime',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('forensic_starttime_finance_paragraphtime').getValue();
					var endDate = Ext.getCmp('forensic_endtime_finance_paragraphtime').getValue();
					if (start > endDate) {
						Ext.getCmp('forensic_starttime_finance_paragraphtime').setValue(endDate);
					}
				}
			}
		});
		var paragraphtime_endtime = new Ext.form.DateField({
			id:'forensic_endtime_finance_paragraphtime',
...
//			forceSelection: true, 
		...
		me.store = Ext.create('Ext.data.Store',{
			fields:['id','num','name','date','ownpersonname','ownperson','areaname','client','commitment',
			        'agentid','agentname','reportif', 'remark','cancelif', 'receivables',
			        'payment','paid','paragraphtime','account_type','discountPrice',
			        'expressnum','expresstype','recive','remarks','remittanceName','remittanceDate','confirm_flag'],
	        start:0,
			limit:15,
			pageSize:15,
			proxy: {
            ...
            listeners:{
            	'beforeload':function(ds, operation, opt){
            		...
            		Ext.apply(me.store.proxy.params, {
            			...
            			forensic_starttime:dateformat(forensic_starttime.getValue()),
            			forensic_endtime:dateformat(forensic_endtime.getValue()),
        				reportif:reportif.getValue(),cancelif:2,agent:agent.getValue(),
        				areacode:areacode.getValue(),
        				commitment:commitment.getValue(),
...
//			mode: 'SINGLE'
       ...
       var pagingToolbar=Ext.getCmp('pagingbarForensicFinance');
...
//		me.bbar = {xtype: 'label',id:'totalBbar_finance_forensic', text: '',
...
//			Ext.getCmp('totalBbar_finance_forensic').setText("共 "+me.store.getCount()+" 条");
//		});
	},
