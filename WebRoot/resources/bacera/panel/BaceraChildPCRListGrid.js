var childPCR="";
Ext.define('Rds.bacera.panel.BaceraChildPCRListGrid', {
	...
	initComponent : function() {
		...
		var childPCR_starttime = new Ext.form.DateField({
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('childPCR_starttime').getValue();
		            var endDate = Ext.getCmp('childPCR_endtime').getValue();
		            if (start > endDate) {
		                Ext.getCmp('childPCR_starttime').setValue(endDate);
		            }
				}
			}
		});
...
//			forceSelection: true, 
		...
		me.store = Ext.create('Ext.data.Store',{
			fields:['id','num','codenum','name','fmname','age','testitems','checkper','phonenum','gender',
			        'date','ownpersonname','ownperson','agentname','reportif','remark','cancelif',
			        'expresstype','receivables','areaname','reportType','orderPlatform','inspectionUnits',
			        'case_type','confirm_flag'],
	        start:0,
			limit:15,
			pageSize:15,
			proxy: {
            ...
            listeners:{
            	'beforeload':function(ds, operation, opt){
            		...
            		childPCR = "num=" + search.getValue().trim() + "&name="
							+ name.getValue().trim() + "&ownperson="
							+ ownperson.getValue().trim() + "&childPCR_starttime="
							+ dateformat(childPCR_starttime.getValue())
							+ "&childPCR_endtime="
							+ dateformat(childPCR_endtime.getValue())
							+ "&reportif=" + reportif.getValue()
							+ "&cancelif=" + cancelif.getValue() + "&agent="
							+ agent.getValue().trim() + "&areacode="
							+ areacode.getValue() + "&checkper="
							+ checkper.getValue().trim() +"&gender="
							+ gender.getValue() + "&inspectionUnits="
							+ inspectionUnits.getValue().trim() + "&reportType="
							+ reportType.getValue().trim() + "&orderPlatform="
							+ orderPlatform.getValue().trim()+"&case_type="
							+ case_type.getValue()+"&codenum="
							+ codenum.getValue().trim()+"&fmname="
							+ fmname.getValue().trim();
            		Ext.apply(me.store.proxy.params, {
		        			num:trim(search.getValue()),
		    				...
		    				orderPlatform:trim(orderPlatform.getValue()),
		    				inspectionUnits:trim(inspectionUnits.getValue()),
		    				reportType:trim(reportType.getValue()),
       ...
       var pagingToolbar=Ext.getCmp('pagingbarChildPCR');
...
//		me.bbar = {xtype: 'label',id:'totalBbarchildPCR', text: '',
//				style:'height:25px;line-height:25px;text-align:right;margin-right:10px;'};
		...
		me.dockedItems = [{
	        ...
	        borderBottomWidth : '0px !important'
	 		...
	 		name:'search',
	 		dock:'top',
	 		items:[reportif,childPCR_starttime,childPCR_endtime,cancelif,checkper,gender]
	 	},{
	 		style : {
		        borderTopWidth : '0px !important',
		        borderBottomWidth : '0px !important'
		 		...
		 		name:'search',
		 		dock:'top',
		 		items:[inspectionUnits,reportType,orderPlatform,case_type,name]
		 	},,{
		 		style : {
			        borderTopWidth : '0px !important',
...
//			Ext.getCmp('totalBbarchildPCR').setText("共 "+me.store.getCount()+" 条");
//		});
	},
	tumorSubExport:function(){
		window.location.href = "bacera/childPCR/exportChildPCR.do?"+childPCR ;
	},
	//新增
	onInsert:function(){
		...
		ownpersonTemp = "";
		confirm_flagTemp="";
		var form = Ext.create("Rds.bacera.form.BaceraChildPCRListForm",{
			region:"center",
			autoScroll : true,
			grid:me
		});
