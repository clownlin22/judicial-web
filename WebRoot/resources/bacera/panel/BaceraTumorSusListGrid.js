var tumorSus="";
Ext.define('Rds.bacera.panel.BaceraTumorSusListGrid', {
	...
	initComponent : function() {
		...
		var tumorSus_starttime = new Ext.form.DateField({
			id:'tumorSus_starttime',
			name : 'tumorSus_starttime',
			width:'20%',
			fieldLabel : '日 期  从',
			labelWidth : 50,
			emptyText : '请选择日期',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('tumorSus_starttime').getValue();
		            var endDate = Ext.getCmp('tumorSus_endtime').getValue();
		            if (start > endDate) {
		                Ext.getCmp('tumorSus_starttime').setValue(endDate);
		            }
				}
			}
		});
		...
		me.store = Ext.create('Ext.data.Store',{
            ...
            listeners:{
            	'beforeload':function(ds, operation, opt){
            		...
            		tumorSus = "num=" + search.getValue().trim() + "&name="
							+ name.getValue().trim() + "&ownperson="
							+ ownperson.getValue().trim() + "&tumorSus_starttime="
							+ dateformat(tumorSus_starttime.getValue())
							+ "&tumorSus_endtime="
							+ dateformat(tumorSus_endtime.getValue())
       ...
       var pagingToolbar=Ext.getCmp('pagingbarTumorSus');
...
//		me.bbar = {xtype: 'label',id:'totalBbarTumorSus', text: '',
//				style:'height:25px;line-height:25px;text-align:right;margin-right:10px;'};
		...
		me.dockedItems = [{
	        ...
	        borderBottomWidth : '0px !important'
	 		...
	 		name:'search',
	 		dock:'top',
	 		items:[reportif,tumorSus_starttime,tumorSus_endtime,cancelif,gender]
	 	},{
	 		style : {
		        borderTopWidth : '0px !important',
...
//			Ext.getCmp('totalBbarTumorSus').setText("共 "+me.store.getCount()+" 条");
//		});
	},
	tumorSubExport:function(){
		window.location.href = "bacera/tumorSus/exportTumorSus.do?"+tumorSus ;
	},
	//新增
	onInsert:function(){
		...
		ownpersonTemp = "";
		confirm_flagTemp="";
		var form = Ext.create("Rds.bacera.form.BaceraTumorSusListForm",{
			region:"center",
			grid:me
		});
