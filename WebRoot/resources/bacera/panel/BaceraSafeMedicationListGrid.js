var safeMed=""
Ext.define('Rds.bacera.panel.BaceraSafeMedicationListGrid', {
...
//		});
		var safeMed_starttime = new Ext.form.DateField({
			id:'safeMed_starttime',
			name : 'safeMed_starttime',
			width:'20%',
			fieldLabel : '日期从',
			labelWidth : 50,
			emptyText : '请选择日期',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('safeMed_starttime').getValue();
		            var endDate = Ext.getCmp('safeMed_endtime').getValue();
		            if (start > endDate) {
		                Ext.getCmp('safeMed_starttime').setValue(endDate);
		            }
				}
			}
		});
		...
		me.store = Ext.create('Ext.data.Store',{
			fields:['id','num','type','name','testitems','date','ownpersonname',
			        'ownperson','agentname','reportif','remark','cancelif','expresstype',
            ...
            listeners:{
            	'beforeload':function(ds, operation, opt){
            		...
            		safeMed = "num=" + search.getValue().trim() + "&name="
		    				+ name.getValue().trim() + "&ownperson="
		    				+ ownperson.getValue().trim() + "&safeMed_starttime="
		    				+ dateformat(safeMed_starttime.getValue())
		    				+ "&safeMed_endtime="
		    				+ dateformat(safeMed_endtime.getValue())
		    				+ "&reportif=" + reportif.getValue()
		    				+ "&cancelif=" + cancelif.getValue() + "&agent="
		    				+ agent.getValue().trim() + "&areacode="
		    				+ areacode.getValue() + "&testitems="
		    				+ testitems.getValue() +"&type="
		    				+ type.getValue();
       ...
       var pagingToolbar=Ext.getCmp('pagingbarSafeMedication');
...
//		me.bbar = {xtype: 'label',id:'totalBbarSafeMed', text: '',style:'height:25px;line-height:25px;text-align:right;margin-right:10px;'};
		me.columns = [
		...
		me.dockedItems = [{
	        ...
	        borderBottomWidth : '0px !important'
	 		...
	 		name:'search',
	 		dock:'top',
	 		items:[reportif,safeMed_starttime,safeMed_endtime,cancelif,testitems,{
            	text:'查询',
            	iconCls:'Find',
            	handler:me.onSearch
            }]
	        ...
	        borderBottomWidth : '1px !important'
	 		...
	 		items:[{
	 			...
	 			text:'导出',
	 			iconCls:'Application',
	 			handler:me.safeMedExport
	 		}]
	 	}];
		me.store.load();
		me.callParent(arguments);
//		me.store.on("load",function(){
//			Ext.getCmp('totalBbarSafeMed').setText("共 "+me.store.getCount()+" 条");
//		});
	},
	safeMedExport:function(){
		window.location.href = "bacera/safeMedication/exportSafeMed.do?"+safeMed ;
	},
	//新增
	onInsert:function(){
		...
		ownpersonTemp="";
		confirm_flagTemp="";
		var form = Ext.create("Rds.bacera.form.BaceraSafeMedicationListForm",{
			region:"center",
			grid:me
		});
	...
	onDelete:function(){
		...
		Ext.MessageBox.confirm("提示", tempStr+"确认删除选中记录？", function (btn) {
	        if("yes"==btn)
	        {
	    		...
	    		Ext.Ajax.request({  
	    			url:"bacera/safeMedication/delete.do", 
	    			method: "POST",
