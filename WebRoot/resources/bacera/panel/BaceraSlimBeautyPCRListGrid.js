var SlimBeautyPCR="";
Ext.define('Rds.bacera.panel.BaceraSlimBeautyPCRListGrid', {
	...
	initComponent : function() {
		...
		var slimBeautyPCR_starttime = new Ext.form.DateField({
			id:'slimBeautyPCR_starttime',
			name : 'slimBeautyPCR_starttime',
			width:'20%',
			fieldLabel : '日 期  从',
			labelWidth : 60,
			emptyText : '请选择日期',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('slimBeautyPCR_starttime').getValue();
		            var endDate = Ext.getCmp('slimBeautyPCR_endtime').getValue();
		            if (start > endDate) {
		                Ext.getCmp('slimBeautyPCR_starttime').setValue(endDate);
		            }
				}
			}
		});
		var slimBeautyPCR_endtime = new Ext.form.DateField({
			id:'slimBeautyPCR_endtime',
			name : 'slimBeautyPCR_endtime',
			width:'20%',
			labelWidth : 20,
			fieldLabel : ' 到 ',
			labelAlign : 'right',
		...
		me.store = Ext.create('Ext.data.Store',{
			fields:['id','num','name','age','testitems','checkper','phonenum','gender','date','ownpersonname',
			        'ownperson','agentname','reportif','remark','cancelif','expresstype','receivables',
			        'areaname','reportType','orderPlatform','inspectionUnits','case_type','confirm_flag'],
	        start:0,
			limit:15,
			pageSize:15,
			proxy: {
                ...
                url: 'bacera/slimBeautyPCR/queryallpage.do',
                params:{
                },
                reader: {
                    type: 'json',
            ...
            listeners:{
            	'beforeload':function(ds, operation, opt){
            		...
            		slimBeautyPCR = "num=" + search.getValue().trim() + "&name="
							+ name.getValue().trim() + "&ownperson="
							+ ownperson.getValue().trim() + "&slimBeautyPCR_starttime="
							+ dateformat(slimBeautyPCR_starttime.getValue())
							+ "&slimBeautyPCR_endtime="
							+ dateformat(slimBeautyPCR_endtime.getValue())
							+ "&reportif=" + reportif.getValue()
							+ "&cancelif=" + cancelif.getValue() + "&agent="
							+ agent.getValue().trim() + "&areacode="
							+ areacode.getValue() + "&checkper="
							+ checkper.getValue() +"&gender="
							+ gender.getValue() + "&inspectionUnits="
							+ inspectionUnits.getValue() + "&reportType="
							+ reportType.getValue() + "&orderPlatform="
							+ orderPlatform.getValue()+"&case_type="
							+ case_type.getValue();
            		Ext.apply(me.store.proxy.params, {
		        			...
		        			slimBeautyPCR_starttime:dateformat(slimBeautyPCR_starttime.getValue()),
		        			slimBeautyPCR_endtime:dateformat(slimBeautyPCR_endtime.getValue()),
		    				reportif:reportif.getValue(),
       ...
       var pagingToolbar=Ext.getCmp('pagingbarSlimBeautyPCR');
...
//		me.bbar = {xtype: 'label',id:'totalBbarslimBeautyPCR', text: '',
//				style:'height:25px;line-height:25px;text-align:right;margin-right:10px;'};
		...
		me.dockedItems = [{
	        ...
	        borderBottomWidth : '0px !important'
	 		...
	 		name:'search',
	 		dock:'top',
	 		items:[reportif,slimBeautyPCR_starttime,slimBeautyPCR_endtime,cancelif,]
	 	},{
	 		style : {
		        borderTopWidth : '0px !important',
		        borderBottomWidth : '0px !important'
		 		...
		 		name:'search',
		 		dock:'top',
		 		items:[inspectionUnits,reportType,orderPlatform,case_type]
		 	},{
		 		style : {
			        borderTopWidth : '0px !important',
	 		...
	 		items:[{
	 			...
	 			text:'导出',
	 			iconCls:'Application',
	 			handler:me.tumorSubExport
	 		}]
	 	}];
		me.store.load();
		me.callParent(arguments);
//		me.store.on("load",function(){
//			Ext.getCmp('totalBbarslimBeautyPCR').setText("共 "+me.store.getCount()+" 条");
//		});
	},
	tumorSubExport:function(){
		window.location.href = "bacera/slimBeautyPCR/exportSlimBeautyPCR.do?"+slimBeautyPCR ;
	},
	//新增
	onInsert:function(){
		...
		ownpersonTemp = "";
		confirm_flagTemp="";
		var form = Ext.create("Rds.bacera.form.BaceraSlimBeautyPCRListForm",{
			region:"center",
			grid:me
		});
	...
	onDelete:function(){
		...
		Ext.MessageBox.confirm("提示", tempStr+"确认删除选中记录？", function (btn) {
	        if("yes"==btn)
	        {
	    		...
	    		Ext.Ajax.request({  
	    			url:"bacera/slimBeautyPCR/delete.do", 
	    			method: "POST",
