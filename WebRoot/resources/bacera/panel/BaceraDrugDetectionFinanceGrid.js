...
Ext.define('Rds.bacera.panel.BaceraDrugDetectionFinanceGrid', {
	...
	initComponent : function() {
		...
		var entrusted_unit = Ext.create('Ext.form.field.Text',{
	    	name:'entrusted_unit',
	    	labelWidth:60,
	    	width:'20%',
	    	fieldLabel:'委托单位'
		...
		var sample_type = Ext.create('Ext.form.field.Text',{
	    	name:'sample_type',
	    	labelWidth:60,
	    	width:'20%',
	    	fieldLabel:'样品类型'
			...
			store: Ext.create(
			        'Ext.data.Store',
			        {
			          ...
			          proxy:{
			        	type: 'jsonajax',
			            actionMethods:{read:'POST'},
			            url:'bacera/drugDetection/queryProgram.do',
			            reader:{
			              type:'json',
			              root:'data'
			            }
		...
		var drugDetection_starttime = new Ext.form.DateField({
			id:'drugDetection_starttime_finance',
			name : 'drugDetection_starttime',
			width:'20%',
			fieldLabel : '日 期  从',
			labelWidth : 70,
			labelAlign : 'left',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('drugDetection_starttime_finance').getValue();
	                var endDate = Ext.getCmp('drugDetection_endtime_finance').getValue();
	                if (start > endDate) {
	                    Ext.getCmp('drugDetection_starttime_finance').setValue(endDate);
	                }
				}
			}
		});
		var drugDetection_endtime = new Ext.form.DateField({
			id:'drugDetection_endtime_finance',
			name : 'drugDetection_endtime',
			width:'20%',
			labelWidth : 20,
			fieldLabel : ' 到 ',
			labelAlign : 'left',
		...
		var paragraphtime_starttime = new Ext.form.DateField({
			id:'drugDetection_starttime_finance_paragraphtime',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('drugDetection_starttime_finance_paragraphtime').getValue();
					var endDate = Ext.getCmp('drugDetection_endtime_finance_paragraphtime').getValue();
					if (start > endDate) {
						Ext.getCmp('drugDetection_starttime_finance_paragraphtime').setValue(endDate);
					}
				}
			}
		});
		var paragraphtime_endtime = new Ext.form.DateField({
			id:'drugDetection_endtime_finance_paragraphtime',
...
//			forceSelection: true, 
		...
		me.store = Ext.create('Ext.data.Store',{
			fields:['id','num','name','program','date','ownpersonname','inspection','entrusted_unit','sample_type',
			        'ownperson','agentname','reportif','remark','cancelif', 'receivables','payment',
			        'paid','discountPrice','paragraphtime','account_type','sample_count',
			        'areaname','remarks','remittanceName','remittanceDate','confirm_flag'],
	        start:0,
			limit:15,
			pageSize:15,
			proxy: {
                type: 'jsonajax',
                actionMethods:{read:'POST'},
                url: 'bacera/drugDetection/queryallpage.do',
                params:{
                },
                reader: {
                    type: 'json',
            ...
            listeners:{
            	'beforeload':function(ds, operation, opt){
            		...
            		Ext.apply(me.store.proxy.params, {
            			...
            			drugDetection_starttime:dateformat(drugDetection_starttime.getValue()),
            			drugDetection_endtime:dateformat(drugDetection_endtime.getValue()),
        				...
        				entrusted_unit:entrusted_unit.getValue().trim(),
        				sample_type:sample_type.getValue().trim()
        				});
        		}
            }
		});
...
//			mode: 'SINGLE'
       ...
       var pagingToolbar=Ext.getCmp('pagingbarDrugDetectionFinance');
		...
		me.bbar = Ext.create('Ext.PagingToolbar', {
			id:'pagingbarDrugDetectionFinance',
			store: me.store,
...
//		me.bbar = {xtype: 'label',id:'totalBbarMedScreen_finance', text: '',style:'height:25px;line-height:25px;text-align:right;margin-right:10px;'};
		me.columns = [
                  ...
                  { text: '被鉴定人',	dataIndex: 'name', menuDisabled:true, width : 120},
                  { text: '送检人',	dataIndex: 'inspection', menuDisabled:true, width : 120},
                  { text: '鉴定事项',	dataIndex: 'program', menuDisabled:true, width : 120},
                  { text: '样品类型',	dataIndex: 'sample_type', menuDisabled:true, width : 120},
                  { text: '样本数',	dataIndex: 'program', menuDisabled:true, width : 120},
		...
		me.dockedItems = [{
	 		...
	 		name:'search',
	 		dock:'top',
	 		items:[search,name,ownperson,areacode,entrusted_unit]
	 	},{
	 		style : {
	        ...
	        borderBottomWidth : '0px !important'
	 		...
	 		name:'search',
	 		dock:'top',
	 		items:[drugDetection_starttime,drugDetection_endtime,program,reportif,sample_type]
	 	},{
	 		style : {
		        borderTopWidth : '0px !important',
		        borderBottomWidth : '0px !important'
		 		...
		 		name:'search',
		 		dock:'top',
		 		items:[paragraphtime_starttime,paragraphtime_endtime,inspection,{
	            	text:'查询',
	            	iconCls:'Find',
	            	handler:me.onSearch
	            }]
...
//			Ext.getCmp('totalBbarMedScreen_finance').setText("共 "+me.store.getCount()+" 条");
//		});
	},
	caseFeeAdd:function(){
