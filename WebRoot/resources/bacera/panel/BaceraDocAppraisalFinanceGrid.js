...
Ext.define('Rds.bacera.panel.BaceraDocAppraisalFinanceGrid', {
	...
	initComponent : function() {
		...
		var docAppraisal_starttime = new Ext.form.DateField({
			id:'docAppraisal_starttime_finance',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('docAppraisal_starttime_finance').getValue();
	                var endDate = Ext.getCmp('docAppraisal_endtime_finance').getValue();
	                if (start > endDate) {
	                    Ext.getCmp('docAppraisal_starttime_finance').setValue(endDate);
	                }
				}
			}
		});
		var docAppraisal_endtime = new Ext.form.DateField({
			id:'docAppraisal_endtime_finance',
		...
		var paragraphtime_starttime = new Ext.form.DateField({
			id:'docAppraisal_starttime_finance_paragraphtime',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('docAppraisal_starttime_finance_paragraphtime').getValue();
					var endDate = Ext.getCmp('docAppraisal_endtime_finance_paragraphtime').getValue();
					if (start > endDate) {
						Ext.getCmp('docAppraisal_starttime_finance_paragraphtime').setValue(endDate);
					}
				}
			}
		});
		var paragraphtime_endtime = new Ext.form.DateField({
			id:'docAppraisal_endtime_finance_paragraphtime',
...
//			mode: 'SINGLE'
       ...
       var pagingToolbar=Ext.getCmp('pagingbarDocFinance');
...
//		me.bbar = {xtype: 'label',id:'totalBbarMedExamine_finance', text: '',style:'height:25px;line-height:25px;text-align:right;margin-right:10px;'};
		me.columns = [
		...
		me.dockedItems = [{
	        ...
	        borderBottomWidth : '0px !important'
	 		...
	 		name:'search',
	 		dock:'top',
	 		items:[docAppraisal_starttime,docAppraisal_endtime,program,reportif]
	 	},{
	 		style : {
		        borderTopWidth : '0px !important',
		        borderBottomWidth : '0px !important'
...
//			Ext.getCmp('totalBbarMedExamine_finance').setText("共 "+me.store.getCount()+" 条");
//		});
	},
	...
	confirmFinanceList:function(){
		...
		{
			ids += selections[i].get("id")+",";
			payment += selections[i].get("payment")+",";
			if(selections[i].get("confirm_flag")=='2'){
				Ext.Msg.alert("提示", "存在已确认案例!");
				return;
			}
	    		...
	    		Ext.Ajax.request({
	    			...
	    			success : function(response, options) {
	    				...
	    				if (response.result) {
	    					//确认成功，生成增值税信息
	    		    		Ext.Ajax.request({
	    		    			url : "bacera/docAppraisal/saveFinanceTax.do",
	    		    			method : "POST",
	    		    			headers : {
	    		    				'Content-Type' : 'application/json'
	    		    			},
	    		    			jsonData : {ids:ids,payment:payment},
	    		    			success : function(response, options) {
	    		    				response = Ext.JSON
	    		    						.decode(response.responseText);
	    		    				if (response) {
