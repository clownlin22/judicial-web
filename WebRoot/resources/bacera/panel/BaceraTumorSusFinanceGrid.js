...
Ext.define('Rds.bacera.panel.BaceraTumorSusFinanceGrid', {
	...
	initComponent : function() {
		...
		var tumorSus_starttime = new Ext.form.DateField({
			id:'tumorSus_starttime_finance',
			name : 'tumorSus_starttime_finance',
        	width:'20%',
			fieldLabel : '日 期  从',
			labelWidth : 50,
			emptyText : '请选择日期',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('tumorSus_starttime_finance').getValue();
	                var endDate = Ext.getCmp('tumorSus_endtime_finance').getValue();
	                if (start > endDate) {
	                    Ext.getCmp('tumorSus_starttime_finance').setValue(endDate);
	                }
				}
			}
		});
		var tumorSus_endtime = new Ext.form.DateField({
			id:'tumorSus_endtime_finance',
			name : 'tumorSus_endtime_finance',
        	width:'20%',
			labelWidth : 20,
			fieldLabel : ' 到 ',
			labelAlign : 'right',
		...
		var paragraphtime_starttime = new Ext.form.DateField({
			id:'tumorSus_starttime_finance_paragraphtime',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('tumorSus_starttime_finance_paragraphtime').getValue();
					var endDate = Ext.getCmp('tumorSus_endtime_finance_paragraphtime').getValue();
					if (start > endDate) {
						Ext.getCmp('tumorSus_starttime_finance_paragraphtime').setValue(endDate);
					}
				}
			}
		});
		var paragraphtime_endtime = new Ext.form.DateField({
			id:'tumorSus_endtime_finance_paragraphtime',
		...
		me.store = Ext.create('Ext.data.Store',{
			fields:['id','num','name','age','testitems','gender','checkper','receivables','payment','paid',
			        ...
			        'remarks','remittanceName','remittanceDate'],
	        start:0,
			limit:15,
			pageSize:15,
			proxy: {
...
//			mode: 'SINGLE'
		...
		//分页的combobox下拉选择显示条数
	     combo = Ext.create('Ext.form.ComboBox',{
	        ...
	        var pagingToolbar=Ext.getCmp('pagingbarTumorSusFinance');
...
//		me.bbar = {xtype: 'label',id:'totalBbarTumorSus_finance', text: '',style:'height:25px;line-height:25px;text-align:right;margin-right:10px;'};
		me.columns = [
...
//			Ext.getCmp('totalBbarTumorSus_finance').setText("共 "+me.store.getCount()+" 条");
//		});
	},
