...
Ext.define('Rds.bacera.panel.BaceraFolicAcidFinanceGrid', {
	...
	initComponent : function() {
		...
		var folicAcid_starttime = new Ext.form.DateField({
			id:'folicAcid_starttime_finance',
			name : 'folicAcid_starttime_finance',
        	width:'20%',
			fieldLabel : '日 期  从',
			labelWidth : 50,
			emptyText : '请选择日期',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('folicAcid_starttime_finance').getValue();
	                var endDate = Ext.getCmp('folicAcid_endtime_finance').getValue();
	                if (start > endDate) {
	                    Ext.getCmp('folicAcid_starttime_finance').setValue(endDate);
	                }
				}
			}
		});
		var folicAcid_endtime = new Ext.form.DateField({
			id:'folicAcid_endtime_finance',
			name : 'folicAcid_endtime_finance',
        	width:'20%',
			labelWidth : 20,
			fieldLabel : ' 到 ',
			labelAlign : 'right',
		...
		var paragraphtime_starttime = new Ext.form.DateField({
			id:'folicAcid_starttime_finance_paragraphtime',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('folicAcid_starttime_finance_paragraphtime').getValue();
					var endDate = Ext.getCmp('folicAcid_endtime_finance_paragraphtime').getValue();
					if (start > endDate) {
						Ext.getCmp('folicAcid_starttime_finance_paragraphtime').setValue(endDate);
					}
				}
			}
		});
		var paragraphtime_endtime = new Ext.form.DateField({
			id:'folicAcid_endtime_finance_paragraphtime',
		...
		me.store = Ext.create('Ext.data.Store',{
			fields:['id','num','name','age','testitems','diagnosis','checkper','agentname','ownpersonname',
			        ...
			        'account_type','reportif','hospital','phonenum','date','remark','cancelif',
...
//			mode: 'SINGLE'
       ...
       var pagingToolbar=Ext.getCmp('pagingbarFolicAcidFinance');
...
//		me.bbar = {xtype: 'label',id:'totalBbarfolicAcid_finance', text: '',style:'height:25px;line-height:25px;text-align:right;margin-right:10px;'};
		me.columns = [
...
//			Ext.getCmp('totalBbarfolicAcid_finance').setText("共 "+me.store.getCount()+" 条");
//		});
	},
	caseFeeAdd:function(){
