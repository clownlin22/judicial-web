...
Ext.define('Rds.bacera.panel.BaceraMedThalassemiaFinanceGrid', {
	...
	initComponent : function() {
		...
		var medThalassemia_starttime = new Ext.form.DateField({
			id:'medThalassemia_starttime_finance',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('medThalassemia_starttime_finance').getValue();
	                var endDate = Ext.getCmp('medThalassemia_endtime_finance').getValue();
	                if (start > endDate) {
	                    Ext.getCmp('medThalassemia_starttime_finance').setValue(endDate);
	                }
				}
			}
		});
		var medThalassemia_endtime = new Ext.form.DateField({
			id:'medThalassemia_endtime_finance',
		...
		var paragraphtime_starttime = new Ext.form.DateField({
			id:'medThalassemia_starttime_finance_paragraphtime',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('medThalassemia_starttime_finance_paragraphtime').getValue();
					var endDate = Ext.getCmp('medThalassemia_endtime_finance_paragraphtime').getValue();
					if (start > endDate) {
						Ext.getCmp('medThalassemia_starttime_finance_paragraphtime').setValue(endDate);
					}
				}
			}
		});
		var paragraphtime_endtime = new Ext.form.DateField({
			id:'medThalassemia_endtime_finance_paragraphtime',
...
//			forceSelection: true, 
		...
		me.store = Ext.create('Ext.data.Store',{
			fields:['id','code','num','name','program','date','ownpersonname','inspection',
...
//			mode: 'SINGLE'
	    ...
	    var pagingToolbar=Ext.getCmp('pagingbarMedThalassemiaFinance');
		...
		me.bbar = Ext.create('Ext.PagingToolbar', {
			id:'pagingbarMedThalassemiaFinance',
			store: me.store,
...
//		me.bbar = {xtype: 'label',id:'totalBbarMedThalassemia_finance', text: '',style:'height:25px;line-height:25px;text-align:right;margin-right:10px;'};
		me.columns = [
                  ...
                  { text: '汇款账户',	dataIndex: 'remittanceName', menuDisabled:true, width : 150,
                    ...
                    },
                    {
                      ...
                      { text: '物流条码',	dataIndex: 'code', menuDisabled:true, width:80},
		...
		me.dockedItems = [{
	 		...
	 		name:'search',
	 		dock:'top',
	 		items:[search,name,ownperson,areacode]
	 	},{
	 		style : {
	        ...
	        borderBottomWidth : '0px !important'
	 		...
	 		name:'search',
	 		dock:'top',
	 		items:[medThalassemia_starttime,medThalassemia_endtime,program,reportif]
	 	},{
	 		style : {
		        borderTopWidth : '0px !important',
		        borderBottomWidth : '0px !important'
		 		...
		 		name:'search',
		 		dock:'top',
		 		items:[paragraphtime_starttime,paragraphtime_endtime,inspection,code,{
	            	text:'查询',
	            	iconCls:'Find',
	            	handler:me.onSearch
	            }]
...
//			Ext.getCmp('totalBbarMedThalassemia_finance').setText("共 "+me.store.getCount()+" 条");
//		});
	},
