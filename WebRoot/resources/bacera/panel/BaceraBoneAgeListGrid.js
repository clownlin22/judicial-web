var boneage="";
//初始化grid，分页查询企业信息
Ext.define('Rds.bacera.panel.BaceraBoneAgeListGrid', {
	...
	initComponent : function() {
		...
		var bone_starttime = new Ext.form.DateField({
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('bone_starttime').getValue();
	                var endDate = Ext.getCmp('bone_endtime').getValue();
	                if (start > endDate) {
	                    Ext.getCmp('bone_starttime').setValue(endDate);
	                }
				}
			}
		});
	...
	//		forceSelection: true, 
		...
		me.store = Ext.create('Ext.data.Store',{
			fields:['id','num','name','date','ownpersonname','ownperson','agentid','agentname','reportif',
			        'remark','cancelif','expresstype','receivables','areaname','confirm_flag'],
	        start:0,
			limit:15,
			pageSize:15,
			proxy: {
            ...
            listeners:{
            	'beforeload':function(ds, operation, opt){
            		...
            		boneage = "num=" + search.getValue().trim() + "&name="
		    				+ name.getValue().trim() + "&ownperson="
		    				+ ownperson.getValue().trim() + "&bone_starttime="
		    				+ dateformat(bone_starttime.getValue())
		    				+ "&bone_endtime="
		    				+ dateformat(bone_endtime.getValue())
        			...
        			Ext.apply(me.store.proxy.params, {
        				...
        				agent:agent.getValue(),areacode:areacode.getValue()});
        		}
            }
		});
       ...
       var pagingToolbar=Ext.getCmp('pagingbarBoneAge');
...
//		me.bbar = {xtype: 'label',id:'totalBbar', text: '',
//				style:'height:25px;line-height:25px;text-align:right;margin-right:10px;'};
		me.columns = [
                  ...
                  { text: '应收款项',	dataIndex: 'receivables', menuDisabled:true, width : 70},
		...
		me.dockedItems = [{
	        ...
	        borderBottomWidth : '1px !important'
	 		...
	 		name:'search',
	 		dock:'top',
	 		items:[reportif,bone_starttime,bone_endtime,cancelif,{
            	text:'查询',
            	iconCls:'Find',
            	handler:me.onSearch
            }]
	 		...
	 		items:[{
	 			...
	 			text:'导出',
	 			iconCls:'Application',
	 			handler:me.boneExport
	 		}]
	 	}];
		me.store.load();
		me.callParent(arguments);
//		me.store.on("load",function(){
//			Ext.getCmp('totalBbar').setText("共 "+me.store.getCount()+" 条");
//		});
	},
	//新增
	onInsert:function(){
		...
		ownpersonTemp="";
		confirm_flagTemp="";
		var form = Ext.create("Rds.bacera.form.BaceraBoneAgeListForm",{
			region:"center",
			grid:me
		});
	...
	boneExport:function(){
		window.location.href = "bacera/boneAge/exportBoneInfo.do?"+boneage ;
	},
	onUpdate:function(){
		var me = this.up("gridpanel");
		var selections =  me.getView().getSelectionModel().getSelection();
