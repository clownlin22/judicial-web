...
Ext.define('Rds.bacera.panel.BaceraSlimBeautyPCRFinanceGrid', {
	...
	initComponent : function() {
		...
		var slimBeautyPCR_starttime = new Ext.form.DateField({
			id:'slimBeautyPCR_starttime_finance',
			name : 'slimBeautyPCR_starttime_finance',
        	width:'20%',
			fieldLabel : '日 期  从',
			labelWidth : 50,
			emptyText : '请选择日期',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('slimBeautyPCR_starttime_finance').getValue();
	                var endDate = Ext.getCmp('slimBeautyPCR_endtime_finance').getValue();
	                if (start > endDate) {
	                    Ext.getCmp('slimBeautyPCR_starttime_finance').setValue(endDate);
	                }
				}
			}
		});
		var slimBeautyPCR_endtime = new Ext.form.DateField({
			id:'slimBeautyPCR_endtime_finance',
			name : 'slimBeautyPCR_endtime_finance',
        	width:'20%',
			labelWidth : 20,
			fieldLabel : ' 到 ',
			labelAlign : 'right',
		...
		var paragraphtime_starttime = new Ext.form.DateField({
			id:'slimBeautyPCR_starttime_finance_paragraphtime',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('slimBeautyPCR_starttime_finance_paragraphtime').getValue();
					var endDate = Ext.getCmp('slimBeautyPCR_endtime_finance_paragraphtime').getValue();
					if (start > endDate) {
						Ext.getCmp('slimBeautyPCR_starttime_finance_paragraphtime').setValue(endDate);
					}
				}
			}
		});
		var paragraphtime_endtime = new Ext.form.DateField({
			id:'slimBeautyPCR_endtime_finance_paragraphtime',
		...
		me.store = Ext.create('Ext.data.Store',{
			fields:['id','num','name','age','testitems','gender','checkper','receivables','payment','paid',
			        'paragraphtime','account_type','phonenum','date','ownpersonname','ownperson',
			        'discountPrice','agentname','reportif','remark','cancelif','remarks','remittanceName',
			        'remittanceDate','reportType', 'orderPlatform','inspectionUnits','case_type','confirm_flag'],
	        start:0,
			limit:15,
			pageSize:15,
	        proxy: {
...
//			mode: 'SINGLE'
		...
		//分页的combobox下拉选择显示条数
	     combo = Ext.create('Ext.form.ComboBox',{
	        ...
	        var pagingToolbar=Ext.getCmp('pagingbarSlimVeautyPCRFinance');
			...
			me.bbar = Ext.create('Ext.PagingToolbar', {
				id:'pagingbarSlimVeautyPCRFinance',
				store: me.store,
...
//		me.bbar = {xtype: 'label',id:'totalBbarSlimBeautyPCR_finance', text: '',
//				style:'height:25px;line-height:25px;text-align:right;margin-right:10px;'};
		...
		me.dockedItems = [{
	        ...
	        borderBottomWidth : '0px !important'
	 		...
	 		name:'search',
	 		dock:'top',
	 		items:[reportif,slimBeautyPCR_starttime,slimBeautyPCR_endtime,checkper,gender]
	 	},{
	 		style : {
		        borderTopWidth : '0px !important',
		        borderBottomWidth : '0px !important'
...
//			Ext.getCmp('totalBbarSlimBeautyPCR_finance').setText("共 "+me.store.getCount()+" 条");
//		});
	},
...
function afterEdit(e,s){
  	Ext.Ajax.request({  
		...
		jsonData: {
			...
			case_type:('1'==s.record.data.case_type)?'减肥美容基因检测':(('2'==s.record.data.case_type)?'个体特征基因检测':'重大慢病基因检测'),
