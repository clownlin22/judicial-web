var medExamine="";
Ext.define('Rds.bacera.panel.BaceraWuHanMedExamineListGrid', {
...
//			            url:'bacera/wuHanMedExamine/queryProgram.do',
//			            reader:{
//			              type:'json',
//			              root:'data'
//			            }
...
//			forceSelection: true, 
            ...
            listeners:{
            	'beforeload':function(ds, operation, opt){
            		...
            		medExamine = "num=" + search.getValue().trim() + "&ownperson="
		    				...
		    				+ (program.getValue()==null?"":program.getValue())+"&name="+name.getValue().trim();
		...
		//分页的combobox下拉选择显示条数
	     combo = Ext.create('Ext.form.ComboBox',{
	        ...
	        var pagingToolbar=Ext.getCmp('pagingbarWuHan');
...
//	   	 	emptyMsg : "没有符合条件的记录"
//        });
//		me.bbar = {xtype: 'label',id:'totalBbar_wuHanMedExamine', text: '',style:'height:25px;line-height:25px;text-align:right;margin-right:10px;'};
		me.columns = [
...
//			Ext.getCmp('totalBbar_wuHanMedExamine').setText("共 "+me.store.getCount()+" 条");
//		});
	},
	//新增
	onInsert:function(){
		...
		ownpersonTemp = "";
		confirm_flagTemp = "";
		var form = Ext.create("Rds.bacera.form.BaceraWuHanMedExamineListForm",{
			region:"center",
			grid:me
		});
	...
	medExamineExport:function(){
		window.location.href = "bacera/wuHanMedExamine/exportMedExamine.do?"+medExamine ;
	},
	onDelete:function(){
		var me = this.up("gridpanel");
		...
		Ext.MessageBox.confirm("提示", tempStr+"确认删除选中记录？", function (btn) {
	        if("yes"==btn)
	        {
	    		...
	    		Ext.Ajax.request({  
	    			url:"bacera/wuHanMedExamine/delete.do", 
	    			method: "POST",
	...
	onCancel:function(){
		...
		Ext.MessageBox.confirm("提示", "确认作废选中记录？", function (btn) {
	        if("yes"==btn)
	        {
	        	Ext.MessageBox.wait('正在保存','请稍后...');
	    		Ext.Ajax.request({  
	    			url:"bacera/wuHanMedExamine/save.do", 
	    			method: "POST",
	    			headers: { 'Content-Type': 'application/json' },
	    			jsonData: values, 
	    			success: function (response, options) {  
