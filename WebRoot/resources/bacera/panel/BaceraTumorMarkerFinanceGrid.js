...
Ext.define('Rds.bacera.panel.BaceraTumorMarkerFinanceGrid', {
	...
	initComponent : function() {
		...
		var TMF_starttime = new Ext.form.DateField({
			id:'TMF_starttime_finance',
			name : 'starttime_finance',
			width:'20%',
			fieldLabel : '日 期  从',
			labelWidth : 50,
			emptyText : '请选择日期',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('TMF_starttime_finance').getValue();
	                var endDate = Ext.getCmp('TMF_endtime_finance').getValue();
	                if (start > endDate) {
	                    Ext.getCmp('TMF_starttime_finance').setValue(endDate);
	                }
				}
			}
		});
		var TMF_endtime = new Ext.form.DateField({
			id:'TMF_endtime_finance',
			name : 'endtime_finance',
			width:'20%',
			labelWidth : 20,
			fieldLabel : ' 到 ',
			labelAlign : 'right',
		...
		var paragraphtime_starttime = new Ext.form.DateField({
			id:'TMF_starttime_finance_paragraphtime',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('TMF_starttime_finance_paragraphtime').getValue();
					var endDate = Ext.getCmp('TMF_endtime_finance_paragraphtime').getValue();
					if (start > endDate) {
						Ext.getCmp('TMF_starttime_finance_paragraphtime').setValue(endDate);
					}
				}
			}
		});
		var paragraphtime_endtime = new Ext.form.DateField({
			id:'TMF_endtime_finance_paragraphtime',
		...
		me.store = Ext.create('Ext.data.Store',{
			fields:['id','num','name','inspection','sex','receivables','payment','paid',
			        'paragraphtime','account_type','date','ownpersonname','ownperson',
			        'discountPrice','agentname','reportif','remark','cancelif','confirm_flag',
			        'remarks','remittanceName','remittanceDate','areaname','areacode'],
	        start:0,
			limit:15,
			pageSize:15,
			proxy: {
            ...
            listeners:{
            	'beforeload':function(ds, operation, opt){
            		...
            		Ext.apply(me.store.proxy.params, {
            			...
            			starttime:dateformat(TMF_starttime.getValue()),
            			endtime:dateformat(TMF_endtime.getValue()),
        				reportif:reportif.getValue(),
        				cancelif:2,
        				inspection:inspection.getValue(),
        				sex:sex.getValue(),
	    				areaname:Ext.getCmp('areacodeTM1').getRawValue(),
...
//			mode: 'SINGLE'
		...
		//分页的combobox下拉选择显示条数
	     combo = Ext.create('Ext.form.ComboBox',{
	        ...
	        var pagingToolbar=Ext.getCmp('pagingbarTumorMakerFinance');
...
//		me.bbar = {xtype: 'label',id:'totalBbarTumorMarker_finance', text: '',style:'height:25px;line-height:25px;text-align:right;margin-right:10px;'};
		me.columns = [
		              { text: '案例编号',	dataIndex: 'num', menuDisabled:true, width : 100,
		                  ...
		                  { text: '检测项目',	dataIndex: 'program', menuDisabled:true, width : 80},
		...
		me.dockedItems = [{
	 		...
	 		name:'search',
	 		dock:'top',
	 		items:[search,name,ownperson,program]
	 	},{
	 		style : {
	        ...
	        borderBottomWidth : '0px !important'
	 		...
	 		name:'search',
	 		dock:'top',
	 		items:[reportif,TMF_starttime,TMF_endtime,areacode]
	 	},{
	 		style : {
		        borderTopWidth : '0px !important',
...
//			Ext.getCmp('totalBbarTumorMarker_finance').setText("共 "+me.store.getCount()+" 条");
//		});
	},
