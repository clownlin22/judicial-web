...
Ext.define('Rds.bacera.panel.BaceraCTDnaExpressGrid', {
	...
	initComponent : function() {
		...
		var ctDNA_starttime_express = new Ext.form.DateField({
			id:'ctDNA_starttime_express',
			name : 'ctDNA_starttime_express',
			width:'20%',
			fieldLabel : '日 期  从',
			labelWidth : 50,
			emptyText : '请选择日期',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('ctDNA_starttime_express').getValue();
		            var endDate = Ext.getCmp('ctDNA_endtime_express').getValue();
		            if (start > endDate) {
		                Ext.getCmp('ctDNA_starttime_express').setValue(endDate);
		            }
				}
			}
		});
		var ctDNA_endtime_express = new Ext.form.DateField({
			id:'ctDNA_endtime_express',
			name : 'ctDNA_endtime_express',
			width:'20%',
			labelWidth : 20,
			fieldLabel : ' 到 ',
			labelAlign : 'right',
...
//			forceSelection: true, 
		...
		me.store = Ext.create('Ext.data.Store',{
			fields:['id','num','sex','name','clinical_diagnosis','histort_effect','areaname','paragraphtime',
			        'date','ownpersonname','ownperson','agentname','reportif','remark','cancelif','finance_remark',
			        'expresstype','expressnum','recive','expresstime','expressremark'],
			proxy: {
                type: 'jsonajax',
            ...
            listeners:{
            	'beforeload':function(ds, operation, opt){
            		...
            		Ext.apply(me.store.proxy.params, {
		        			...
		        			ctDNA_starttime:dateformat(ctDNA_starttime_express.getValue()),
		        			ctDNA_endtime:dateformat(ctDNA_endtime_express.getValue()),
		...
		me.bbar = {xtype: 'label',id:'totalBbarCTDna_express', text: '',
		...
		me.dockedItems = [{
	        ...
	        borderBottomWidth : '0px !important'
	 		...
	 		name:'search',
	 		dock:'top',
	 		items:[reportif,ctDNA_starttime_express,ctDNA_endtime_express,sex,expressnum,{
            	text:'查询',
            	iconCls:'Find',
            	handler:me.onSearch
            }]
		...
		me.store.on("load",function(){
			Ext.getCmp('totalBbarCTDna_express').setText("共 "+me.store.getCount()+" 条");
		});
	},
	onSearch:function(){
