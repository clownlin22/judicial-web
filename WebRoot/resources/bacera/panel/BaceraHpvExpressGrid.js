...
Ext.define('Rds.bacera.panel.BaceraHpvExpressGrid', {
	...
	initComponent : function() {
		...
		var hpv_starttime = new Ext.form.DateField({
			id:'hpv_starttime_express',
			name : 'hpv_starttime_express',
			width:'20%',
			fieldLabel : '日 期  从',
			labelWidth : 70,
			emptyText : '请选择日期',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('hpv_starttime_express').getValue();
	                var endDate = Ext.getCmp('hpv_endtime_express').getValue();
	                if (start > endDate) {
	                    Ext.getCmp('hpv_starttime_express').setValue(endDate);
	                }
				}
			}
		});
		var hpv_endtime = new Ext.form.DateField({
			id:'hpv_endtime_express',
			name : 'hpv_endtime_express',
			width:'20%',
			labelWidth : 20,
			fieldLabel : ' 到 ',
			labelAlign : 'right',
		...
		me.store = Ext.create('Ext.data.Store',{
				fields:['id','num','name','age','gender','paragraphtime','account_type',
				        'expresstype','expressnum','recive','date','ownpersonname','ownperson',
...
//		me.bbar = {xtype: 'label',id:'totalBbarHpv_express', text: '',
...
//			Ext.getCmp('totalBbarHpv_express').setText("共 "+me.store.getCount()+" 条");
//		});
	},
	onSearch:function(){
