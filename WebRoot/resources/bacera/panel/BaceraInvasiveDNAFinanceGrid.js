...
Ext.define('Rds.bacera.panel.BaceraInvasiveDNAFinanceGrid', {
	...
	initComponent : function() {
		...
		var invasive_starttime = new Ext.form.DateField({
			id:'invasiveDNA_finance_starttime',
			name : 'invasiveDNA_finance_starttime',
        	width:'20%',
			fieldLabel : '受理日期从',
			labelWidth : 70,
			emptyText : '请选择日期',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('invasiveDNA_finance_starttime').getValue();
	                var endDate = Ext.getCmp('invasiveDNA_finance_endtime').getValue();
	                if (start > endDate) {
	                    Ext.getCmp('invasiveDNA_finance_starttime').setValue(endDate);
	                }
				}
			}
		});
		var invasive_endtime = new Ext.form.DateField({
			id:'invasiveDNA_finance_endtime',
			name : 'invasiveDNA_finance_endtime',
        	width:'20%',
			labelWidth : 20,
			fieldLabel : ' 到 ',
			labelAlign : 'right',
		...
		var paragraphtime_starttime = new Ext.form.DateField({
			id:'invasiveDNA_starttime_finance_paragraphtime',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('invasiveDNA_starttime_finance_paragraphtime').getValue();
					var endDate = Ext.getCmp('invasiveDNA_endtime_finance_paragraphtime').getValue();
					if (start > endDate) {
						Ext.getCmp('invasiveDNA_starttime_finance_paragraphtime').setValue(endDate);
					}
				}
			}
		});
		var paragraphtime_endtime = new Ext.form.DateField({
			id:'invasiveDNA_endtime_finance_paragraphtime',
		...
		var consigning_starttime = new Ext.form.DateField({
			id:'consigning_starttime_finance',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('consigning_starttime_finance').getValue();
	                var endDate = Ext.getCmp('consigning_endtime_finance').getValue();
	                if (start > endDate) {
	                    Ext.getCmp('consigning_starttime_finance').setValue(endDate);
	                }
				}
			}
		});
		var consigning_endtime = new Ext.form.DateField({
			id:'consigning_endtime_finance',
...
//			forceSelection: true, 
		...
		me.store = Ext.create('Ext.data.Store',{
			fields:['id','num','consigningDate','fatherName','motherName','fatherType','gestational','client','date','ownpersonname',
			        'ownperson','agentname','reportif','remark','cancelif','receivables','payment','paid',
			        'paragraphtime','discountPrice','account_type','expressnum','expresstype','recive','remarks','remittanceName','remittanceDate','confirm_flag'],
	        start:0,
			limit:15,
			pageSize:15,
			proxy: {
...
//			mode: 'SINGLE'
      ...
      var pagingToolbar=Ext.getCmp('pagingbarinvasiveDnaFinance');
		...
		me.bbar = Ext.create('Ext.PagingToolbar', {
			id:'pagingbarinvasiveDnaFinance',
			store: me.store,
...
//		me.bbar = {xtype: 'label',id:'totalBbarInvasiveDNA_finance', text: '',
//				style:'height:25px;line-height:25px;text-align:right;margin-right:10px;'};
		...
		me.dockedItems = [{
	 		...
	 		name:'search',
	 		dock:'top',
	 		items:[search,ownperson,areacode,agent,reportif]
	 	},{
	 		style : {
	        ...
	        borderBottomWidth : '0px !important'
	 		...
	 		name:'search',
	 		dock:'top',
	 		items:[client,fatherType,invasive_starttime,invasive_endtime]
	 	},{
	 		style : {
		        borderTopWidth : '0px !important',
		        borderBottomWidth : '0px !important'
		 		...
		 		name:'search',
		 		dock:'top',
		 		items:[consigning_starttime,consigning_endtime,paragraphtime_starttime,paragraphtime_endtime]
	 	},{
	 		style : {
		        borderTopWidth : '0px !important',
		        borderBottomWidth : '0px !important'
...
//			Ext.getCmp('totalBbarInvasiveDNA_finance').setText("共 "+me.store.getCount()+" 条");
//		});
	},
	caseFeeAdd:function(){
