...
Ext.define('Rds.bacera.panel.BaceraHpvFinanceGrid', {
	...
	initComponent : function() {
		...
		var hpv_starttime = new Ext.form.DateField({
			id:'hpv_starttime_finance',
			name : 'hpv_starttime_finance',
			width:'20%',
			fieldLabel : '日 期  从',
			labelWidth : 50,
			emptyText : '请选择日期',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('hpv_starttime_finance').getValue();
	                var endDate = Ext.getCmp('hpv_endtime_finance').getValue();
	                if (start > endDate) {
	                    Ext.getCmp('hpv_starttime_finance').setValue(endDate);
	                }
				}
			}
		});
		var hpv_endtime = new Ext.form.DateField({
			id:'hpv_endtime_finance',
			name : 'hpv_endtime_finance',
			width:'20%',
			labelWidth : 20,
			fieldLabel : ' 到 ',
			labelAlign : 'right',
		...
		var paragraphtime_starttime = new Ext.form.DateField({
			id:'hpv_starttime_finance_paragraphtime',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('hpv_starttime_finance_paragraphtime').getValue();
					var endDate = Ext.getCmp('hpv_endtime_finance_paragraphtime').getValue();
					if (start > endDate) {
						Ext.getCmp('hpv_starttime_finance_paragraphtime').setValue(endDate);
					}
				}
			}
		});
		var paragraphtime_endtime = new Ext.form.DateField({
			id:'hpv_endtime_finance_paragraphtime',
		...
		me.store = Ext.create('Ext.data.Store',{
			fields:['id','num','name','age','gender','receivables','payment','paid',
			        'paragraphtime','account_type','date','ownpersonname','ownperson',
			        'discountPrice','agentname','reportif','remark','cancelif',
			...
			proxy: {
                type: 'jsonajax',
                actionMethods:{read:'POST'},
                url: 'bacera/hpv/queryallpage.do',
                params:{
                },
                reader: {
                    type: 'json',
            ...
            listeners:{
            	'beforeload':function(ds, operation, opt){
            		...
            		Ext.apply(me.store.proxy.params, {
            			...
            			hpv_starttime:dateformat(hpv_starttime.getValue()),
            			hpv_endtime:dateformat(hpv_endtime.getValue()),
...
//			mode: 'SINGLE'
       ...
       var pagingToolbar=Ext.getCmp('pagingbarHPVFinance');
...
//		me.bbar = {xtype: 'label',id:'totalBbarHpv_finance', text: '',style:'height:25px;line-height:25px;text-align:right;margin-right:10px;'};
		me.columns = [
		...
		me.dockedItems = [{
	        ...
	        borderBottomWidth : '0px !important'
	 		...
	 		name:'search',
	 		dock:'top',
	 		items:[reportif,hpv_starttime,hpv_endtime,gender]
	 	},{
	 		style : {
		        borderTopWidth : '0px !important',
...
//			Ext.getCmp('totalBbarHpv_finance').setText("共 "+me.store.getCount()+" 条");
//		});
	},
