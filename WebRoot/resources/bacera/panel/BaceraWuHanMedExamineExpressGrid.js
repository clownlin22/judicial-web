...
Ext.define('Rds.bacera.panel.BaceraWuHanMedExamineExpressGrid', {
	...
	initComponent : function() {
		...
		var medExamine_starttime = new Ext.form.DateField({
			id:'wuHanMedExamine_starttime_express',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('wuHanMedExamine_starttime_express').getValue();
	                var endDate = Ext.getCmp('wuHanMedExamine_endtime_express').getValue();
	                if (start > endDate) {
	                    Ext.getCmp('wuHanMedExamine_starttime_express').setValue(endDate);
	                }
				}
			}
		});
		var medExamine_endtime = new Ext.form.DateField({
			id:'wuHanMedExamine_endtime_express',
		...
		me.store = Ext.create('Ext.data.Store',{
            ...
            listeners:{
            	'beforeload':function(ds, operation, opt){
            		...
            		medExamineWuHanExpress = "num=" + search.getValue().trim() + "&ownperson="
		    				...
		    				+ (program.getValue()==null?"":program.getValue())+"&name="
		    				+ name.getValue().trim()+"&expressnum="
...
//		me.bbar = {xtype: 'label',id:'totalBbarWuHanMedExamine_express', text: '',style:'height:25px;line-height:25px;text-align:right;margin-right:10px;'};
		me.columns = [
		...
		me.dockedItems = [{
	        ...
	        borderBottomWidth : '0px !important'
	 		...
	 		name:'search',
	 		dock:'top',
	 		items:[medExamine_starttime,medExamine_endtime,program,reportif]
	 	},{
	 		style : {
		        borderTopWidth : '0px !important',
		        borderBottomWidth : '0px !important'
		 		...
		 		name:'search',
		 		dock:'top',
		 		items:[express_starttime,express_endtime,expressnum,{
	            	text:'查询',
	            	iconCls:'Find',
	            	handler:me.onSearch
	            }]
	 		...
	 		items:[{
	 			...
	 			text:'导出',
	 			iconCls:'Application',
	 			handler:me.medExamineWuHanExpressExport
	 		}]
	 	}];
		me.store.load();
		me.callParent(arguments);
//		me.store.on("load",function(){
//			Ext.getCmp('totalBbarWuHanMedExamine_express').setText("共 "+me.store.getCount()+" 条");
//		});
	},
	medExamineWuHanExpressExport:function(){
		window.location.href = "bacera/wuHanMedExamine/exportMedExamine.do?"+medExamineWuHanExpress ;
	},
	caseCode:function(){
