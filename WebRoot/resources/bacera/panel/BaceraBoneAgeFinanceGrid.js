...
//初始化grid，分页查询企业信息
Ext.define('Rds.bacera.panel.BaceraBoneAgeFinanceGrid', {
	...
	initComponent : function() {
		...
		var bone_starttime = new Ext.form.DateField({
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('bone_starttime_finance').getValue();
	                var endDate = Ext.getCmp('bone_endtime_finance').getValue();
	                if (start > endDate) {
	                    Ext.getCmp('bone_starttime_finance').setValue(endDate);
	                }
				}
			}
		});
		var bone_endtime = new Ext.form.DateField({
			id:'bone_endtime_finance',
			name : 'bone_endtime',
        	width:'20%',
			labelWidth : 20,
			fieldLabel : ' 到 ',
		...
		var paragraphtime_starttime = new Ext.form.DateField({
			id:'bone_starttime_finance_paragraphtime',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('bone_starttime_finance_paragraphtime').getValue();
					var endDate = Ext.getCmp('bone_endtime_finance_paragraphtime').getValue();
					if (start > endDate) {
						Ext.getCmp('bone_starttime_finance_paragraphtime').setValue(endDate);
					}
				}
			}
		});
		var paragraphtime_endtime = new Ext.form.DateField({
			id:'bone_endtime_finance_paragraphtime',
		...
		me.store = Ext.create('Ext.data.Store',{
			fields:['id','num','name','date','ownpersonname','ownperson','areaname','agentname','reportif',
			        'remark','cancelif','receivables','payment','paid','paragraphtime','account_type','confirm_flag',
			        'discountPrice','expressnum','expresstype','recive','remarks','remittanceName','remittanceDate'],
	        start:0,
			limit:15,
			pageSize:15,
			proxy: {
            ...
            listeners:{
            	'beforeload':function(ds, operation, opt){
            		...
            		Ext.apply(me.store.proxy.params, {num:search.getValue(),name:name.getValue(),ownperson:ownperson.getValue(),
        				bone_starttime:dateformat(bone_starttime.getValue()),bone_endtime:dateformat(bone_endtime.getValue()),
...
//			mode: 'SINGLE'
        ...
        var pagingToolbar=Ext.getCmp('pagingbarBoneAgeFinance');
...
//		me.bbar = {xtype: 'label',id:'totalBbar_finance', text: '',
//				style:'height:25px;line-height:25px;text-align:right;margin-right:10px;'};
		...
		me.dockedItems = [{
	        ...
	        borderBottomWidth : '0px !important'
	 		...
	 		name:'search',
	 		dock:'top',
	 		items:[reportif,bone_starttime,bone_endtime]
	 	},{
	 		style : {
		        borderTopWidth : '0px !important',
...
//			Ext.getCmp('totalBbar_finance').setText("共 "+me.store.getCount()+" 条");
//		});
	},
	caseFeeAdd:function(){
...
//			if(!('masl' == usercode || 'xieh' == usercode  || 'sq_wangyan'==usercode ))
//				return false;
		}
	}
});
