var folicAcid=""
Ext.define('Rds.bacera.panel.BaceraFolicAcidListGrid', {
	...
	initComponent : function() {
		...
		var folicAcid_starttime = new Ext.form.DateField({
			id:'folicAcid_starttime',
			name : 'folicAcid_starttime',
			width:'20%',
			fieldLabel : '日 期  从',
			labelWidth : 50,
			labelAlign : 'right',
			...
			listeners:{
				'select':function(){
					var start = Ext.getCmp('folicAcid_starttime').getValue();
		            var endDate = Ext.getCmp('folicAcid_endtime').getValue();
		            if (start > endDate) {
		                Ext.getCmp('folicAcid_starttime').setValue(endDate);
		            }
				}
			}
		});
		var folicAcid_endtime = new Ext.form.DateField({
			id:'folicAcid_endtime',
			name : 'folicAcid_endtime',
			width:'20%',
			labelWidth : 20,
			fieldLabel : ' 到 ',
			labelAlign : 'right',
...
//			forceSelection: true, 
		...
		me.store = Ext.create('Ext.data.Store',{
			fields:['id','num','name','age','sex','testitems','diagnosis','checkper','phonenum','date','hospital',
			        'agentname','reportif','ownpersonname','ownperson','remark','cancelif',
            ...
            listeners:{
            	'beforeload':function(ds, operation, opt){
            		...
            		folicAcid = "num=" + search.getValue().trim() + "&name="
		        			+ name.getValue().trim() + "&ownperson="
		        			+ ownperson.getValue().trim() + "&folicAcid_starttime="
		        			+ dateformat(folicAcid_starttime.getValue())
		        			+ "&folicAcid_endtime="
		        			+ dateformat(folicAcid_endtime.getValue())
       ...
       var pagingToolbar=Ext.getCmp('pagingbarFolicAcid');
...
//		me.bbar = {xtype: 'label',id:'totalBbarFolicAcid', text: '',
//				style:'height:25px;line-height:25px;text-align:right;margin-right:10px;'};
		...
		me.dockedItems = [{
	        ...
	        borderBottomWidth : '0px !important'
	 		...
	 		name:'search',
	 		dock:'top',
	 		items:[reportif,folicAcid_starttime,folicAcid_endtime,cancelif,checkper,{
            	text:'查询',
            	iconCls:'Find',
            	handler:me.onSearch
            }]
	 		...
	 		items:[{
	 			...
	 			text:'导出',
	 			iconCls:'Application',
	 			handler:me.folicAcidExport
	 		}]
	 	}];
		me.store.load();
		me.callParent(arguments);
//		me.store.on("load",function(){
//			Ext.getCmp('totalBbarFolicAcid').setText("共 "+me.store.getCount()+" 条");
//		});
	},
	folicAcidExport:function(){
		window.location.href = "bacera/folicAcid/exportFolicAcid.do?"+folicAcid ;
	},
	//新增
	onInsert:function(){
		...
		ownpersonTemp = "";
		confirm_flagTemp="";
		var form = Ext.create("Rds.bacera.form.BaceraFolicAcidListForm",{
			region:"center",
			grid:me
		});
	...
	onDelete:function(){
		...
		Ext.MessageBox.confirm("提示", tempStr+"确认删除选中记录？", function (btn) {
	        if("yes"==btn)
	        {
	    		...
	    		Ext.Ajax.request({  
	    			url:"bacera/folicAcid/delete.do", 
	    			method: "POST",
