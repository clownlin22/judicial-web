var medScreen="";
Ext.define('Rds.bacera.panel.BaceraMedScreenListGrid', {
...
//			forceSelection: true, 
		...
		me.store = Ext.create('Ext.data.Store',{
			fields:['id','num','name','date','program','inspection','agentname','ownpersonname','ownperson',
            ...
            listeners:{
            	'beforeload':function(ds, operation, opt){
            		...
            		medScreen = "num=" + search.getValue().trim() + "&ownperson="
		    				+ ownperson.getValue().trim() + "&medScreen_starttime="
		    				+ dateformat(medScreen_starttime.getValue())
		    				+ "&medScreen_endtime="
		    				+ dateformat(medScreen_endtime.getValue())
	   ...
	   //添加下拉显示条数菜单选中事件
	        ...
	        var pagingToolbar=Ext.getCmp('pagingbarMedScreen');
...
//		me.bbar = {xtype: 'label',id:'totalBbar_medScreen', text: '',style:'height:25px;line-height:25px;text-align:right;margin-right:10px;'};
		me.columns = [
		...
		me.dockedItems = [{
	 		...
	 		style : {
		        ...
		        borderBottomWidth : '0px !important'
		 		...
		 		name:'search',
		 		dock:'top',
		 		items:[medScreen_starttime,medScreen_endtime,cancelif,program,inspection,{
	            	text:'查询',
	            	iconCls:'Find',
	            	handler:me.onSearch
	            }]
	 		...
	 		items:[{
	 			...
	 			text:'导出',
	 			iconCls:'Application',
	 			handler:me.medScreenExport
	 		}]
	 	}];
		me.store.load();
		me.callParent(arguments);
//		me.store.on("load",function(){
//			Ext.getCmp('totalBbar_medScreen').setText("共 "+me.store.getCount()+" 条");
//		});
	},
	//新增
	onInsert:function(){
		...
		ownpersonTemp = "";
		confirm_flagTemp="";
		var form = Ext.create("Rds.bacera.form.BaceraMedScreenListForm",{
			region:"center",
			grid:me
		});
	...
	medScreenExport:function(){
		window.location.href = "bacera/medScreen/exportMedScreen.do?"+medScreen ;
	},
	onDelete:function(){
		var me = this.up("gridpanel");
		...
		Ext.MessageBox.confirm("提示", tempStr+"确认删除选中记录？", function (btn) {
	        if("yes"==btn)
	        {
	    		...
	    		Ext.Ajax.request({  
	    			url:"bacera/medScreen/delete.do", 
	    			method: "POST",
