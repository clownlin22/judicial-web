 ...
 * @author yuanxiaobo
 */
Ext.define("Rds.finance.panel.FinancelContractPlanGridPanel",{
...
//	                  },
	...
	createFinanceDaily:function(){
		...
		var contract_id=selections[0].get("contract_id");
		var contract_program=selections[0].get("contract_program");
		Ext.Msg.show({
			title : '提示(合同项目：'+contract_program+')',
			msg : '确定生成该合同汇款?',
			width : 300,
			...
			fn : function(buttonId, text, opt) {
				if (buttonId == 'ok') {
					...
					Ext.Ajax.request({
								url : "judicial/finance/createContractFinanceDaily.do",
								method : "POST",
								headers : {
									'Content-Type' : 'application/json'
								},
								jsonData : {"contract_id":contract_id},
...
//							totalProperty : 'count'
	...
	onInsert:function(){
		...
		ownpersonTemp='';
		ownaddressTemp='';
		var form = Ext.create("Rds.finance.form.FinanceRemittanceConcatForm", {
					region : "center",
					autoScroll : true,
					grid : me
				});
	...
	onUpdate:function(){
		...
		ownpersonTemp = selections[0].get("contract_userid");
		ownaddressTemp = selections[0].get("contract_areacode");
		var form = Ext.create("Rds.finance.form.FinanceRemittanceConcatUpdateForm", {
					region : "center",
					autoScroll : true,
					grid : me
				});
