 ...
 */
var i = 0;
Ext.define("Rds.appraisal.panel.AppraisalExamineGridPanel",{
...
//							value : Ext.Date.add(new Date(), Ext.Date.DAY, -7),
							...
							listeners:{
								'select':function(){
									var start = Ext.getCmp('accept_date_starttime1').getValue();
				                    var endDate = Ext.getCmp('accept_date_endtime1').getValue();
				                    if (start > endDate) {
				                        Ext.getCmp('accept_date_starttime1').setValue(endDate);
				                    }
								}
							}
						});
						var accept_date_endtime = new Ext.form.DateField({
							id:'accept_date_endtime1',
						...
						var identify_date_start = new Ext.form.DateField({
							...
							listeners:{
								'select':function(){
									var start = Ext.getCmp('identify_date_start2').getValue();
				                    var endDate = Ext.getCmp('identify_date_end2').getValue();
				                    if (start > endDate) {
				                        Ext.getCmp('identify_date_start2').setValue(endDate);
				                    }
								}
							}
						});
						var identify_date_end = new Ext.form.DateField({
							id:'identify_date_end2',
						...
						var flag_status = new Ext.form.field.ComboBox({
							...
							store : new Ext.data.ArrayStore(
									{
										fields : ['Name','Code' ],
										data : [['录入待审核',4],['录入审核未通过',5],['录入审核通过',6] ]
									}),
							value : '',
							mode : 'local',
							name : 'flag_status',
