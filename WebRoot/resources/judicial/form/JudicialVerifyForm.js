...
var img_count = 0;
Ext.define('Rds.judicial.form.JudicialVerifyForm', {
					...
					items : [{
							...
							},
							{
								...
								items : [
				                    ...
				                    store: new Ext.data.ArrayStore({
				                        fields: ['Name', 'Code'],
				                        data: [['申报户口', '申报户口'], ['迁户口', '迁户口'], ['办理出生证明', '办理出生证明'], ['其他', '其他']]
				                    })          ,
				                    mode: 'local',
				                    name: 'purpose'
								...
								items : [ Ext.create('Ext.form.ComboBox', {
									fieldLabel: "CMA<span style='color:red'>*</span>",
				                    labelWidth : 80,
				                    editable:false,
				                    triggerAction: 'all',
				                    displayField: 'Name',
				...
				items : [
				        {
							...
							items :[{
							    ...
							    listeners:{
							        click:function(){
							        	var me = this.up("form");
							        	console.log(sample_code_length);
							        	if(sample_code_length > 0)
							        	{
							        		test();
								        	sample_code_length--;
							        	}
							        		...
							        		me.add({
												...
												items:[{
													...
													items:[{
															...
															items:[{    columnWidth : .5,
																		xtype : "textfield",
																		fieldLabel : "校验条形码<span style='color:red'>*</span>",
																	    allowBlank  : false,//不允许为空  
																        blankText   : "不能为空",//错误提示信息，默认为This field is required!  
																		labelAlign: 'right',
																		maxLength :50,
																		labelWidth : 80,
																		name : 'sample_code_verify',
																		listeners: {  
																			'render' : function() { 
...
//		});
	...
	onVerify : function() {
		...
		var sample_code = values["sample_code"];
		var sample_code_verify=values["sample_code_verify"];
		if(!(undefined == sample_code_verify)){
			if(!(sample_code instanceof Array) && !(sample_code_verify instanceof Array))
			{
				if(sample_code != sample_code_verify)
				{
					Ext.MessageBox.alert("提示信息", "校验条形码和案例条形码不一致！");
					return;
				}
			}else if(sample_code instanceof Array && !(sample_code_verify instanceof Array))
			{
				Ext.MessageBox.alert("提示信息", "校验条形码和案例条形码不一致！");
				return;
			}else if(!(sample_code instanceof Array) && sample_code_verify instanceof Array)
			...
			{
				if(sample_code.sort().toString() != sample_code_verify.sort().toString())
				{
					Ext.MessageBox.alert("提示信息", "校验条形码和案例条形码不一致！");
					return;
				}
							...
							items : [{
								...
								items : [{
									...
									items : [{																		columnWidth : .509,
										...
										labelWidth : 60,
										fieldLabel : '家庭地址',
										labelAlign : 'right',										value : items[i].address,
