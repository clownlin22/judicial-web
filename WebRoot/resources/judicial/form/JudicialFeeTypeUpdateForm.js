...
//	remoteSort : true
//});
Ext.define("Rds.judicial.form.JudicialFeeTypeUpdateForm", {
...
//			id:'case_receiver_update_alcohol',
			...
			store : Ext.create("Ext.data.Store",{
				...
				proxy : {
					type : "ajax",
					url : "judicial/dicvalues/getAllManager.do?area_id="+ownpersonTemp,
					reader : {
						type : "json"
					}
				}
		...
		me.items = [{
			...
			border : false,
			autoHeight : true,
			items : [{xtype : 'hiddenfield',name:"area_id"},
			         {xtype : 'hiddenfield',name:"type_id"},
			         {xtype : 'hiddenfield',name:"feetype"},
			         saleStore
//			         {
//						xtype : "combo",
//						fieldLabel : '销售经理',
...
//					}
	...
	onSave : function() {
		...
		Ext.Ajax.request({
					url : "judicial/feestandard/update.do",
					method : "POST",
					headers : {
	...
	listeners : {
		'afterrender' : function() {
			...
			var area_id = values["area_id"];
			var feetype = values["feetype"];
//			saleStore.load();
			var area_id_update = me.down("combo[name=area_id_update]");
			area_id_update.setValue(area_id);
			var feetype_update = me.down("combo[name=feetype_update]");
			feetype_update.setValue(parseInt(feetype));
		}
	}
});