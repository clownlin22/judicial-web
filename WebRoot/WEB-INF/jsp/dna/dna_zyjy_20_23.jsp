		      ...
		      <c:if test="${empty caseModel.accept_time|| caseModel.accept_time =='' }"> 
					 宿迁子渊司法鉴定所[<c:out value="${fn:substring(caseModel.addextnew_time, 0, 4)}" />]法物鉴字第${caseModel.case_code}号 
				         ...
				         <hr/>
				          ...
				          <div class="page_center">${lenthatelier}-STR基因座分型结果表</div>
		      ...
		      <c:if test="${empty caseModel.accept_time|| caseModel.accept_time =='' }"> 
					 宿迁子渊司法鉴定所[<c:out value="${fn:substring(caseModel.addextnew_time, 0, 4)}" />]法物鉴字第${caseModel.case_code}号 
		                 ...
		                 <div class="pageem content paragraph" style="margin-top: 40px;">
		                     D19S433等${lenthatelier}个STR基因座均为人类的遗传学标记，遵循孟德尔遗传定律，联合应用可进行亲权鉴定。上述${lenthatelier}个STR基因座检测结果显示：除${type.result.compareResultModel.unmatched_node}基因座以外，${type.sampleModel.child_info.sample_username}的等位基因可从${type.sampleModel.father_info.sample_username}和${type.sampleModel.mother_info.sample_username}的基因型中找到来源。经计算，累积亲权指数大于10000。
		                 </div>
				   </div>
				   <div class="page">
				          <div class="page_title_name">
				         ...
				         <hr/>
				          ...
				          <c:forEach items="${type.sampleModel.sampleResults}" var="list" varStatus="status" begin="20" step="1"><tr>
				            ...
				            <c:if test="${type.result.compareResultModel.unmatched_count>0}">
				                       <div class="pageem content paragraph">
							             D19S433等${lenthatelier}个STR基因座均为人类的遗传学标记，遵循孟德尔遗传定律，联合应用可进行亲权鉴定。上述${lenthatelier}个STR基因座检测结果显示：除${type.result.compareResultModel.unmatched_node}基因座以外，${type.sampleModel.child_info.sample_username}的等位基因可从${type.sampleModel.father_info.sample_username}的基因型中找到来源。
							          </div>
				              </c:if>
				              <c:if test="${type.result.compareResultModel.unmatched_count==0}">
				                       <div class="pageem content paragraph">
							             D19S433等${lenthatelier}个STR基因座均为人类的遗传学标记，遵循孟德尔遗传定律，联合应用可进行亲权鉴定。上述${lenthatelier}个STR基因座检测结果显示：${type.sampleModel.child_info.sample_username}的等位基因可从${type.sampleModel.father_info.sample_username}的基因型中找到来源。
							          </div>
				              </c:if>
				        </div>
				        <div class="page">
					 ...
					 宿迁子渊司法鉴定所[<c:out value="${fn:substring(caseModel.addextnew_time, 0, 4)}" />]法物鉴字第${caseModel.case_code}号 
					         ...
					         <table class="table border_no_top" cellSpacing="0" border="solid" cellPadding="0">
				                <c:forEach items="${type.sampleModel.sampleResults}" var="list" varStatus="status" begin="20" step="1"><tr>
						          ...
						          <div class="pageem content paragraph">
						            D19S433等${lenthatelier}个STR基因座均为人类的遗传学标记，遵循孟德尔遗传定律，联合应用可进行亲权鉴定。上述${lenthatelier}个STR基因座检测结果显示：${type.sampleModel.child_info.sample_username}在${type.result.compareResultModel.unmatched_node}基因座上的等位基因不能从${type.sampleModel.father_info.sample_username}的基因型中找到来源。
						          </div>
						          <div class="pageem content paragraph">
						                     依据现有资料和DNA分析结果，排除${type.sampleModel.father_info.sample_username}是${type.sampleModel.child_info.sample_username}的生物学${type.sampleModel.father_info.sample_callname}。
						          </div> 
