             ...
             <div class="page">
		      ...
		      <c:if test="${empty caseModel.accept_time|| caseModel.accept_time =='' }"> 
		          ...
		          <div class="spanel"><span class="p_title">委&ensp;托&ensp;人：</span>${caseModel.client}</div>
		           <div class="spanel"><span class="p_title">委托事项：</span><c:if test="${type.sampleModel.mother_info.sample_code!=''&&type.sampleModel.mother_info.sample_code!=null}">对${type.sampleModel.father_info.sample_username}、${type.sampleModel.mother_info.sample_username}与${type.sampleModel.child_info.sample_username}之间有无亲生血缘关系的鉴定。</c:if><c:if test="${type.sampleModel.mother_info.sample_code==''||type.sampleModel.mother_info.sample_code==null}">对${type.sampleModel.father_info.sample_username}与${type.sampleModel.child_info.sample_username}之间有无亲生血缘关系的鉴定。</c:if></div>
		           <div class="spanel"><span class="p_title"> 委托日期：</span><c:if test="${not empty caseModel.accept_time|| caseModel.accept_time!=''}">${caseModel.accept_time}</c:if><c:if test="${empty caseModel.accept_time|| caseModel.accept_time =='' }"> ${caseModel.addextnew_time}</c:if></div>
		          <div class="spanel"><span class="p_title">受理日期：</span><c:if test="${not empty caseModel.accept_time|| caseModel.accept_time!=''}">${caseModel.accept_time}</c:if>
		      <c:if test="${empty caseModel.accept_time|| caseModel.accept_time =='' }">${caseModel.addextnew_time}</c:if>
			  </div>
		           <div class="spanel"><span class="p_title">鉴定材料：</span>${type.sampleModel.father_info.sample_username}<c:if test="${type.sampleModel.mother_info.sample_code!=''&&type.sampleModel.mother_info.sample_code!=null}">、${type.sampleModel.mother_info.sample_username}</c:if>与${type.sampleModel.child_info.sample_username}的血样。</div>
				    <div class="spanel"><span class="p_title">鉴定日期：</span><c:if test="${not empty caseModel.accept_time|| caseModel.accept_time!=''}">${caseModel.accept_time}-${caseModel.close_time}</c:if>
		      <c:if test="${empty caseModel.accept_time|| caseModel.accept_time =='' }">${caseModel.addextnew_time}-${caseModel.close_time}</c:if></div>
		          <div class="spanel  intent"><span class="p_title">鉴定地点：</span>宿迁子渊司法鉴定所</div>
		          <div class="spanel"><span class="p_title">被鉴定人概况</span></div>
		          <div align=center>
		           <table class="table_check table_people" cellSpacing="0" cellPadding="0" >
				  ...
				  ${caseModel.client}为${type.sampleModel.child_info.sample_username}申报户口（鉴定目的）需要明确${caseModel.client}是否是${type.sampleModel.child_info.sample_username}生物学<c:if test="${type.sampleModel.mother_info.sample_code!=''&&type.sampleModel.mother_info.sample_code!=null}">父母</c:if><c:if test="${type.sampleModel.mother_info.sample_code==''||type.sampleModel.mother_info.sample_code==null}">${type.sampleModel.father_info.sample_callname}</c:if>,故委托宿迁子渊司法鉴定所进行鉴定。于<c:if test="${not empty caseModel.accept_time|| caseModel.accept_time!=''}">${caseModel.accept_time}</c:if><c:if test="${empty caseModel.accept_time|| caseModel.accept_time =='' }">${caseModel.addextnew_time}</c:if>提取了${type.sampleModel.father_info.sample_username}的血样、<c:if test="${type.sampleModel.mother_info.sample_code!=''&&type.sampleModel.mother_info.sample_code!=null}">${type.sampleModel.mother_info.sample_username}的血样</c:if>与${type.sampleModel.child_info.sample_username}的血样。
		          </div>
		          <div class="panel_title">三、资料摘要</div>
		          <div class="pageem content paragraph">
		          <span>${type.sampleModel.father_info.sample_username}的样本编号为${type.sampleModel.father_info.sample_code}，</span>
		          <span><c:if test="${type.sampleModel.mother_info.sample_code!=''&&type.sampleModel.mother_info.sample_code!=null}">${type.sampleModel.mother_info.sample_username}的样本编号为${type.sampleModel.mother_info.sample_code}，  
	           	     </c:if>${type.sampleModel.child_info.sample_username}的样本编号为${type.sampleModel.child_info.sample_code}，</span>
		          <span><c:if test="${not empty caseModel.accept_time|| caseModel.accept_time!=''}">${caseModel.accept_time}-${caseModel.close_time}送至实验室。
			      </c:if></span>
			      <span><c:if test="${empty caseModel.accept_time|| caseModel.accept_time =='' }"> ${caseModel.addextnew_time}-${caseModel.close_time}送至实验室。</c:if>
				  </span>
		          </div>
...
<c:if test="${not empty caseModel.accept_time|| caseModel.accept_time!=''}">
		      ...
		      <c:if test="${empty caseModel.accept_time|| caseModel.accept_time =='' }"> 
				 ...
				 <div class="page_title">共&ensp;3&ensp;页第&ensp;2&ensp;页</div>
		          ...
		          <div class="pageem content paragraph">1.参照《法庭科学&ensp;DNA&ensp;实验室检验规范》（GA/T&ensp;383-2014），直接取直径1.2mm血斑纸片各1张（根据试剂盒说明书要求） </div>
                  <div class="pageem content paragraph">2.用<c:if test="${reagent_name=='MR20' }">GOLDENEYE<sup>TM</sup></c:if><c:if test="${reagent_name=='MR21' }">MICROREADER<sup>TM</sup></c:if><c:if test="${reagent_name=='S20A(MR20)' }">GOLDENEYE<sup>TM</sup></c:if><c:if test="${reagent_name=='SUBO21' }">21plex</c:if>荧光标记STR复合扩增试剂盒<c:if test="${reagent_name=='MR21' }" >(阅微基因技术有限公司)</c:if><c:if test="${reagent_name=='MR20' }" >(阅微基因技术有限公司)</c:if><c:if test="${reagent_name=='SUBO21' }">(江苏苏博生物医学科技南京有限公司)</c:if><c:if test="${reagent_name=='S20A(MR20)' }">(基点认知技术（北京）有限公司)</c:if>进行复合PRC扩增（操作步骤按试剂盒说明），并设立阴性及阳性对照。</div>		        
				  <c:if test="${type.sampleModel.mother_info.sample_code==''||type.sampleModel.mother_info.sample_code==null}">
		          <div class="pageem content paragraph">3.扩增产物用ABI3130xl遗传分析仪（ABI）进行毛细管电泳、分离并采用GeneMapper®ID软件进行基因型分析。</div>
		          </c:if>
		      ...
		      <c:if test="${empty caseModel.accept_time|| caseModel.accept_time =='' }"> 
					 宿迁子渊司法鉴定所[<c:out value="${fn:substring(caseModel.addextnew_time, 0, 4)}" />]法物鉴字第${caseModel.case_code}号 
				         ...
				         <div class="pageem content paragraph"><span>（二）鉴定结果</span></div>
				          ...
				          <table class="table table_people" cellSpacing="0"  cellPadding="0">
				           <tr><th style="width:37%">基因位点</div>
						   </th><th><div>${type.sampleModel.father_info.sample_username}</div><div class="no_margin">${type.sampleModel.father_info.sample_code}</div></th><th><div>${type.sampleModel.child_info.sample_username}</div><div class="no_margin">${type.sampleModel.child_info.sample_code}</div></th><th><div>${type.sampleModel.mother_info.sample_username}</div><div class="no_margin">${type.sampleModel.mother_info.sample_code}</div></th></tr>
...
<c:if test="${not empty caseModel.accept_time|| caseModel.accept_time!=''}">
		     ...
		     <c:if test="${type.result_type=='passed_2'}">
		          <c:if test="${type.result.compareResultModel.ext_flag=='N'}">         
				        <div class="pageem content paragraph"><span>（二）检验结果</span></div>
				         <div class="pageem content first-content">
				           STR基因分型结果：
				          </div>
				          <div class="page_center">STR基因座分型结果表33</div><table class="table table_people" cellSpacing="0"  cellPadding="0">
				             <tr><th>基因座</th><th><div>${type.sampleModel.father_info.sample_callname}：${type.sampleModel.father_info.sample_username}</div><div class="no_margin">${type.sampleModel.father_info.sample_code}</div></th><th><div>孩子：${type.sampleModel.child_info.sample_username}</div><div class="no_margin">${type.sampleModel.child_info.sample_code}</div></th><th>亲权值</th></tr>
				             <c:forEach items="${type.sampleModel.sampleResults}" var="list" varStatus="status" begin="0" end="19" step="1"><tr>
				                  <c:forEach items="${list}" var="map">
										<c:choose>  
										   ...
										   <c:when test="${map.key!='name'&&map.key!='pipi'}"> 
										       <td>
											    <c:if test="${fn:substringAfter(map.value,',')==''}">
											        <table style='width:100%;'>
												        <tr >
		      ...
		      <c:if test="${empty caseModel.accept_time|| caseModel.accept_time =='' }"> 
					 宿迁子渊司法鉴定所[<c:out value="${fn:substring(caseModel.addextnew_time, 0, 4)}" />]法物鉴字第${caseModel.case_code}号 
				         ...
				         <hr/>
					          <div class="panel_title">五、分析说明</div>
					          <div class="pageem content paragraph">
					            D19S433等20个STR基因座均为人类的遗传学标记，遵循孟德尔遗传定律，联合应用可进行亲权鉴定，其累积非父排除概率大于0.9999。</div>
					          <div class="pageem content paragraph">
					                                     综上鉴定结果分析：${type.sampleModel.child_info.sample_username}的等位基因可从${type.sampleModel.father_info.sample_username}的基因型中找到来源。经计算，累积亲权指数为${type.result.pi}（注：大于10000）。                        
					            </div>
					         <div class="panel_title">六、鉴定意见</div> 
					          <div class="pageem content paragraph">
					         ...
					         <c:forEach items="${pers}" var="per" varStatus="status" >
					          ...
					          <div class="ending_mark">
					              <div>注：被鉴定人及相关证件电子照片见附件</div>
					          </div>
					    </div>
		          </c:if>
		           <c:if test="${type.result.compareResultModel.ext_flag=='Y'}">
					 ...
					 宿迁子渊司法鉴定所[<c:out value="${fn:substring(caseModel.addextnew_time, 0, 4)}" />]法物鉴字第${caseModel.case_code}号 
				       ...
				       <div class="pageem content paragraph"><span>（二）鉴定结果</span></div>
				         <div class="pageem content first-content">
				          ...
				          <div class="page_center">39-STR基因座分型结果表@</div>
				          <table class="table table_people" cellSpacing="0" border="solid" cellPadding="0">
					 ...
					 宿迁子渊司法鉴定所[<c:out value="${fn:substring(caseModel.addextnew_time, 0, 4)}" />]法物鉴字第${caseModel.case_code}号 
				         ...
				         <div class="pageem content paragraph"><span>（二）鉴定结果</span></div>
						         <div class="pageem content first-content">
						          ...
						          <div class="page_center">20-STR基因座分型结果表否定</div>
