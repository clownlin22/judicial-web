...
<html>
<head>
<title>宿迁子渊司法鉴定所[2015]法物鉴字第${type.case_code}号</title>
...
<body style="width: 100%;">
           <c:forEach items="${printtables}" var="type" >
					                 ...
					                 <table cellspacing="0" cellpadding="0" style="text-align:center;width: 97%;font-size:28px;">
					                      ...
					                      <c:forEach items="${type.piInfoModels}" var="pi">
					                       <tr><td style="border-right:solid">${pi.param_type}</td>
					                           <td  style="border-right:solid"><c:if test="${fn:substringAfter(pi.parent,',')==''}">
					                           ...
					                           <td style="border-right:solid"><c:if test="${fn:substringAfter(pi.child,',')==''}">
											        <table style='width:100% ;border:none;'>
												        <tr>
													        <td style='text-align: right;width:33.3%'>
					                      ...
					                      <tfoot style="text-align:right;">
					                           ...
					                           <th colspan="2"  style="border-top:solid;"><c:if test="${type.pi_num==0}">${type.pi}</c:if><c:if test="${type.pi_num!=0}">${type.pi_str}×10<sup>${type.pi_num}</sup></c:if></th>
					                           </tr>
					                            <tr>
		         ...
		         <c:if test="${type.model_type=='passed_3'}">
		         <div class="page">
		                   <div class="panel font_bold" style="text-align:center;">${type.case_code}</div>
					                 <table cellspacing="0" cellpadding="0" style="text-align:center;width: 97%;font-size:28px;">
					                      <thead class="table_thead">
					                      ...
					                      <c:forEach items="${type.piInfoModels}" var="pi">
					                       <tr><td style="border-right:solid">${pi.param_type}</td>
					                          <td  style="border-right:solid"><c:if test="${fn:substringAfter(pi.parent,',')==''}">
											    ...
											    <td style="border-right:solid"><c:if test="${fn:substringAfter(pi.parent2,',')==''}">
											        <table style='width:100% ;border:none;'>
												        <tr>
													        <td style='text-align: right;width:33.3%'>
								                                 ${fn:substringBefore(pi.parent2,",")}
															</td>
															<td style='text-align: center;width:33.3%'>,
															</td>
															<td style='text-align:left;width:33.3%'>
											    ...
											    <c:if test="${fn:substringAfter(pi.parent2,',')!=''}">
												        <table style='width:100% ;border:none;'>
													        <tr>
														        <td style='text-align: right;width:33.3%'>
									                                ${fn:substringBefore(pi.parent2,",")}
					                      ...
					                      <tfoot style="text-align:right;">
					                           <tr>
					                           <th colspan="6"    style="border-top:solid;">累积PI</th>
					                           <th colspan="2"  style="border-top:solid;"><c:if test="${type.pi_num==0}">${type.pi}</c:if><c:if test="${type.pi_num!=0}">${type.pi_str}×10<sup>${type.pi_num}</sup></c:if></th>
					                           </tr>
					                            <tr>
					                               <th colspan="6"  style="border-bottom:solid;">RCP</th>
					                               <th colspan="2"  style="border-bottom:solid;">${type.rcp}</th>
					                           </tr>
					                      </tfoot>
					                 </table>
