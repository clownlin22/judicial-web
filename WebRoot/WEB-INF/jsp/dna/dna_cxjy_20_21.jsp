		      ...
		      <c:if test="${empty caseModel.accept_time|| caseModel.accept_time =='' }"> 
					      宿迁子渊司法鉴定所[<c:out value="${fn:substring(caseModel.addextnew_time, 0, 4)}" />]
					         ...
					         <hr/>
						          ...
						          <c:forEach items="${pers}" var="per" varStatus="status" ><c:if test="${status.index==0}">
						          <div class="ending_name" style="margin-top:150px;">鉴定人：${per.identify_per}</div>
					          <div class="ending_num">
					                    《司法鉴定人执业证》证号：${per.identify_code}
					          </div></c:if>
					           <c:if test="${status.index>0}">
