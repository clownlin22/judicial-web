    ...
    <c:forEach items="${caseModel.results}" var="type" >
             <div class="page">
		      ...
		      <c:if test="${empty caseModel.accept_time|| caseModel.accept_time =='' }"> 
		          ...
		          <div class="pageem content paragraph">2.取上述检材的DNA提取产物适量，用<c:if test="${reagent_name=='SUBO21'}">21plex</c:if>荧光标记STR复合扩增试剂盒<c:if test="${reagent_name=='SUBO21'}">(江苏苏博生物医学科技南京有限公司)</c:if><c:if test="${reagent_name=='MR21' }">MICROREADER<sup>TM</sup></c:if>进行10μL体系PCR扩增（操作步骤按试剂盒说明），并设立阴性及阳性对照。</div>
		          <c:if test="${type.sampleModel.mother_info.sample_code==''||type.sampleModel.mother_info.sample_code==null}">
