...
<body>
   ...
   <div class="title_remark right">
      单号：${relay_code}    案例数量：${countSample}
   </div> 
   <div style="height: 1000px;">
   <table cellSpacing="0" cellPadding="0" border="solid;" class="tb_code">
       <tr><td colspan="4" style="font-family: '黑体';height: 30px;">案例编号</td></tr>
       <c:choose>
	       ...
	       <c:forEach items="${relaySampleInfos1}" var="sample" begin="0" step="1" varStatus="status"><tr></tr>
	      		<tr>
					<td>${sample.case_code}</td>
					<td>${relaySampleInfos2[status.index].case_code}</td>
					<td>${relaySampleInfos3[status.index].case_code}</td>
					<td>${relaySampleInfos4[status.index].case_code}</td>
				 </tr>
	       </c:forEach>
		 </c:otherwise>
	 </c:choose> 
   ...
   <div class="paragraph">${relay_remark}</div>
   </div>
   <table class="tb_per">
       <tr><td>交接日期:</td><td style="width:30%">${relay_time}</td></tr>
